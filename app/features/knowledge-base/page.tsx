...
import { ArrowRight, Check, ChevronRight, Menu, X, Search, Globe, Zap, RefreshCw, FileText } from 'lucide-react';
...
export default function KnowledgeBaseFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Knowledge Base</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><Search className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">AI learns from their website.<span className="block mt-2 text-[#fafaf9]/40">No manual training needed.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Just enter the business website URL. Our AI scrapes services, pricing, hours, FAQs, and more—then uses that knowledge to answer caller questions accurately.</p>
              <div className="mt-8 space-y-3">
                {['Automatic website scraping', 'Extracts services, pricing, hours', 'Answers FAQs accurately', 'Updates when website changes'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                ...
                <div className="p-3 rounded-lg bg-white/[0.03] mb-4">
                  <p className="text-xs text-[#fafaf9]/40 mb-1">Source</p>
                  <p className="text-sm font-mono">smithplumbing.com</p>
                </div>
                <div className="space-y-3">
                  <div className="p-3 rounded-lg bg-white/[0.03]">
                    <p className="text-xs text-[#fafaf9]/40 mb-1">Services Detected</p>
                    <p className="text-sm">Plumbing repair, drain cleaning, water heater installation, pipe replacement...</p>
                  </div>
                  <div className="p-3 rounded-lg bg-white/[0.03]">
                    <p className="text-xs text-[#fafaf9]/40 mb-1">Business Hours</p>
                    <p className="text-sm">Mon-Fri 8am-6pm, Sat 9am-3pm, Emergency 24/7</p>
                  </div>
                  <div className="p-3 rounded-lg bg-white/[0.03]">
                    <p className="text-xs text-[#fafaf9]/40 mb-1">Service Area</p>
                    <p className="text-sm">Atlanta metro, Marietta, Roswell, Alpharetta</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-white/[0.06]">
                  <RefreshCw className="h-4 w-4 text-emerald-400" />
                  <p className="text-xs text-[#fafaf9]/40">Last updated: 2 hours ago</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {[
              { step: '1', title: 'Enter website URL', desc: 'When setting up a new client, just paste their website address.' },
              { step: '2', title: 'AI scrapes content', desc: 'Our system reads every page—services, about, FAQ, contact, etc.' },
              { step: '3', title: 'Answers questions', desc: 'AI uses this knowledge to give accurate, business-specific responses.' },
            ].map((item) => (
              <div key={item.step} className="text-center">
                <div className="flex h-12 w-12 mx-auto items-center justify-center rounded-full bg-emerald-500 text-[#050505] font-bold mb-4">{item.step}</div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-2xl font-semibold mb-6">What it learns</h2>
              <div className="grid grid-cols-2 gap-4">
                {['Services offered', 'Pricing info', 'Business hours', 'Service areas', 'FAQs', 'Team info', 'Contact details', 'Policies'].map((item) => (
                  <div key={item} className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-400" /><span className="text-sm">{item}</span></div>
                ))}
              </div>
            </div>
            <div className="p-6 rounded-xl border border-white/[0.06] bg-white/[0.02]">
              <h3 className="font-semibold mb-4">Custom knowledge too</h3>
              <p className="text-sm text-[#fafaf9]/50 mb-4">Beyond website scraping, you can add custom information that isn&apos;t on their site—special promotions, internal policies, or specific talking points.</p>
              <div className="p-3 rounded-lg bg-white/[0.03]">
                <p className="text-xs text-[#fafaf9]/40 mb-1">Custom Note</p>
                <p className="text-sm">&quot;Always mention our 10% senior discount and free estimates on jobs over $500&quot;</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              { title: 'Industries', desc: '12+ industry presets', href: '/features/industries' },
              { title: 'Auto Provisioning', desc: '60-second client setup', href: '/features/auto-provisioning' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Zero manual training. AI just gets it.</h2>
          <p className="mt-4 text-[#fafaf9]/50">Automatic knowledge base included in all plans.</p>
