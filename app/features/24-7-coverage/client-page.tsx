...
import { ArrowRight, Check, ChevronRight, Menu, X, Clock, Moon, Sun, Phone } from 'lucide-react';
...
export default function Coverage247FeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">24/7 Coverage</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><Clock className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Never miss a call.<span className="block mt-2 text-[#fafaf9]/40">24/7/365.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">AI answers every call instantly—nights, weekends, holidays. No sick days, no vacations, no tired voices at 2am. Just consistent, professional service around the clock.</p>
              <div className="mt-8 space-y-3">
                {['Always on, no exceptions', 'Same quality at 2pm or 2am', 'Holiday coverage included', 'No overtime or staffing costs'].map((item) => (
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 rounded-xl bg-white/[0.03] text-center">
                    ...
                    <p className="text-2xl font-bold">9am</p>
                    <p className="text-xs text-[#fafaf9]/40">Business hours</p>
                    <p className="text-emerald-400 text-sm mt-2">✓ AI answers</p>
                  </div>
                  <div className="p-4 rounded-xl bg-white/[0.03] text-center">
                    <Moon className="h-8 w-8 text-blue-400 mx-auto mb-2" />
                    <p className="text-2xl font-bold">2am</p>
                    <p className="text-xs text-[#fafaf9]/40">After hours</p>
                    <p className="text-emerald-400 text-sm mt-2">✓ AI answers</p>
                  </div>
                  <div className="p-4 rounded-xl bg-white/[0.03] text-center">
                    <p className="text-2xl mb-2">🎄</p>
                    <p className="text-lg font-bold">Christmas</p>
                    <p className="text-xs text-[#fafaf9]/40">Holidays</p>
                    <p className="text-emerald-400 text-sm mt-2">✓ AI answers</p>
                  </div>
                  <div className="p-4 rounded-xl bg-white/[0.03] text-center">
                    <p className="text-2xl mb-2">☀️</p>
                    <p className="text-lg font-bold">Sunday</p>
                    <p className="text-xs text-[#fafaf9]/40">Weekends</p>
                    <p className="text-emerald-400 text-sm mt-2">✓ AI answers</p>
                  </div>
                </div>
                <div className="mt-4 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-center">
                  <p className="text-sm text-emerald-400">365 days • 8,760 hours • Never sleeps</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="p-6 rounded-xl border border-white/[0.06] bg-white/[0.02]">
              <h3 className="font-semibold mb-4">The problem with traditional receptionists</h3>
              <div className="space-y-3">
                {['Only available 40 hours/week', 'Sick days and vacations', 'Overtime costs for after-hours', 'Inconsistent quality when tired', 'No holiday coverage'].map((item) => (
                  <div key={item} className="flex items-center gap-2 text-sm text-[#fafaf9]/50"><span className="text-red-400">✗</span>{item}</div>
                ))}
              </div>
            </div>
            <div className="p-6 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.03]">
              <h3 className="font-semibold mb-4 text-emerald-400">AI receptionist advantage</h3>
              <div className="space-y-3">
                {['Available 168 hours/week', 'Never calls in sick', 'No overtime, flat monthly rate', 'Consistent quality every call', 'Full holiday coverage'].map((item) => (
                  <div key={item} className="flex items-center gap-2 text-sm"><span className="text-emerald-400">✓</span>{item}</div>
                ))}
              </div>
            </div>
          </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: 'Business Hours', desc: 'Custom greetings by time', href: '/features/business-hours' },
              { title: 'Urgency Detection', desc: 'Prioritize emergency calls', href: '/features/urgency-detection' },
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Round-the-clock coverage. Zero extra cost.</h2>
          <p className="mt-4 text-[#fafaf9]/50">24/7 coverage included in all plans.</p>
