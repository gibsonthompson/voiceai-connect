...
import { ArrowRight, Check, ChevronRight, Menu, X, Clock, Sun, Moon, Calendar } from 'lucide-react';
...
export default function BusinessHoursFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Business Hours</span></div></div></div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Different hours, different greetings.<span className="block mt-2 text-[#fafaf9]/40">AI adapts automatically.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Set business hours for each client. AI uses different greetings and behaviors during vs. after hours. &quot;Thanks for calling, we&apos;re open!&quot; vs. &quot;We&apos;re currently closed but I can help you...&quot;</p>
              <div className="mt-8 space-y-3">
                {['Custom hours per client', 'Different greetings by time', 'Holiday hours support', 'Timezone-aware'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <h3 className="font-medium mb-4">Business Hours Config</h3>
                <div className="space-y-3">
                  {[
                    { day: 'Mon-Fri', hours: '8:00 AM - 6:00 PM', status: 'open' },
                    { day: 'Saturday', hours: '9:00 AM - 2:00 PM', status: 'open' },
                    { day: 'Sunday', hours: 'Closed', status: 'closed' },
                  ].map((item) => (
                    <div key={item.day} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03]">
                      <span className="font-medium">{item.day}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-[#fafaf9]/50">{item.hours}</span>
                        <span className={`px-2 py-0.5 rounded text-xs ${item.status === 'open' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{item.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="mt-4 p-4 rounded-lg bg-white/[0.03]">
                  <div className="flex items-center gap-2 mb-2"><Sun className="h-4 w-4 text-amber-400" /><span className="text-sm font-medium">During Hours Greeting</span></div>
                  <p className="text-xs text-[#fafaf9]/50">&quot;Thank you for calling! How can I help you today?&quot;</p>
                </div>
                <div className="mt-3 p-4 rounded-lg bg-white/[0.03]">
                  <div className="flex items-center gap-2 mb-2"><Moon className="h-4 w-4 text-blue-400" /><span className="text-sm font-medium">After Hours Greeting</span></div>
                  <p className="text-xs text-[#fafaf9]/50">&quot;We&apos;re currently closed, but I can take a message or help with general questions.&quot;</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              { title: 'Appointments', desc: 'Intelligent scheduling', href: '/features/appointments' },
              { title: 'Knowledge Base', desc: 'AI learns schedules', href: '/features/knowledge-base' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Context-aware greetings. Automatically.</h2>
          <p className="mt-4 text-[#fafaf9]/50">Business hours included in all plans.</p>
