...
import { ArrowRight, Check, ChevronRight, Menu, X, Phone, MapPin, Globe, Zap } from 'lucide-react';
...
export default function PhoneNumbersFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Phone Numbers</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><Phone className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Dedicated phone numbers.<span className="block mt-2 text-[#fafaf9]/40">Provisioned instantly.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Every client gets their own dedicated phone number. Choose local area codes or toll-free. Numbers are live in seconds—no paperwork, no waiting.</p>
              <div className="mt-8 space-y-3">
                {['Local numbers in any US area code', 'Toll-free (800, 888, etc.) available', 'Numbers provisioned in under 60 seconds', 'Port existing numbers (Pro+)'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <h3 className="font-medium mb-4">Number Selection</h3>
                <div className="space-y-3">
                  {[
                    { type: 'Local', number: '(404) 555-1234', area: 'Atlanta, GA' },
                    { type: 'Local', number: '(770) 555-5678', area: 'Marietta, GA' },
                    { type: 'Toll-Free', number: '(888) 555-9012', area: 'Nationwide' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                      <div>
                        <p className="font-mono font-semibold">{item.number}</p>
                        <p className="text-xs text-[#fafaf9]/40">{item.area}</p>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs ${item.type === 'Toll-Free' ? 'bg-amber-500/10 text-amber-400' : 'bg-emerald-500/10 text-emerald-400'}`}>{item.type}</span>
                    </div>
                  ))}
                </div>
                <button className="w-full mt-4 py-3 rounded-lg bg-emerald-500 text-[#050505] font-medium">Provision Instantly</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-semibold text-center mb-12">Number options</h2>
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              { icon: MapPin, title: 'Local Numbers', desc: 'Match the client\'s area code for a local presence. Builds trust with callers.', badge: 'All Plans' },
              { icon: Globe, title: 'Toll-Free Numbers', desc: '800, 888, 877, and more. Professional nationwide presence.', badge: 'Pro+' },
              { icon: Phone, title: 'Number Porting', desc: 'Keep existing numbers. We handle the transfer seamlessly.', badge: 'Pro+' },
            ].map((item) => (
              <div key={item.title} className="p-6 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500/10"><item.icon className="h-5 w-5 text-emerald-400" /></div>
                  <span className="px-2 py-1 rounded text-xs bg-white/[0.05]">{item.badge}</span>
                </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: '24/7 Coverage', desc: 'AI answers around the clock', href: '/features/24-7-coverage' },
              { title: 'Auto Provisioning', desc: '60-second client setup', href: '/features/auto-provisioning' },
              { title: 'Call Recordings', desc: 'Every call recorded', href: '/features/call-recordings' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Phone numbers ready in seconds</h2>
          <p className="mt-4 text-[#fafaf9]/50">Local numbers included. Toll-free and porting on Pro+ plans.</p>
