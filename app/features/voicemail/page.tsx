...
import { ArrowRight, Check, ChevronRight, Menu, X, Voicemail, FileText, Bell, Clock } from 'lucide-react';
...
export default function VoicemailFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Voicemail Transcription</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><Voicemail className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Read voicemails.<span className="block mt-2 text-[#fafaf9]/40">Don&apos;t listen to them.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Every voicemail is automatically transcribed and sent as a text notification. Skim in seconds what would take minutes to listen to. Plus full audio available if needed.</p>
              <div className="mt-8 space-y-3">
                {['Instant text transcription', 'Audio recording saved', 'Sent via SMS/email', 'Searchable voicemail archive'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <div className="flex items-center gap-2 mb-4">
                  <Voicemail className="h-5 w-5 text-emerald-400" />
                  <h3 className="font-medium">New Voicemail</h3>
                  <span className="text-xs text-[#fafaf9]/40 ml-auto">2 min ago</span>
                </div>
                <div className="p-4 rounded-lg bg-white/[0.03] mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center">👤</div>
                    <div><p className="font-medium text-sm">John Smith</p><p className="text-xs text-[#fafaf9]/40">(555) 123-4567</p></div>
                  </div>
                  <div className="flex items-center gap-2 mt-3">
                    <div className="flex-1 h-1 bg-emerald-500/30 rounded-full"><div className="h-full w-1/3 bg-emerald-500 rounded-full" /></div>
                    <span className="text-xs text-[#fafaf9]/40">0:23</span>
                  </div>
                </div>
                <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                  <p className="text-xs text-emerald-400 mb-1">📝 Transcription</p>
                  <p className="text-sm">&quot;Hi, this is John Smith calling about my appointment tomorrow. I need to push it back an hour if possible. Please call me back at this number. Thanks!&quot;</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              { title: 'Transcripts', desc: 'Full call transcription', href: '/features/transcripts' },
              { title: 'Call Recordings', desc: 'Audio playback available', href: '/features/call-recordings' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Voicemails, readable in seconds</h2>
          <p className="mt-4 text-[#fafaf9]/50">Transcription included in all plans.</p>
