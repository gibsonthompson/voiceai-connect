...
import { ArrowRight, Check, ChevronRight, Menu, X, BrainCircuit, MessageSquare, Sparkles, Zap, Shield, Globe } from 'lucide-react';
...
export default function AiIntelligenceFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">AI Intelligence</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><BrainCircuit className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Powered by GPT-4.<span className="block mt-2 text-[#fafaf9]/40">Not a simple chatbot.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Our AI doesn&apos;t just follow scripts. It understands context, handles complex questions, and has natural conversations that feel genuinely human.</p>
              <div className="mt-8 space-y-3">
                {['Understands context and nuance', 'Handles multi-turn conversations', 'Adapts to caller personality', 'Knows when to escalate to human'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <div className="space-y-4">
                  <div className="flex gap-3"><div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center shrink-0">👤</div><div className="p-3 rounded-xl bg-white/[0.05] text-sm">&quot;Hi, I need to reschedule my appointment but I also wanted to ask about your pricing for a bigger project.&quot;</div></div>
                  <div className="flex gap-3 justify-end"><div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-sm max-w-[85%]">&quot;Of course! Let me help you with both. First, what date works better for your appointment? And while we find a time, tell me about your project so I can give you accurate pricing.&quot;</div><div className="h-8 w-8 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0"><Sparkles className="h-4 w-4 text-emerald-400" /></div></div>
                </div>
                <div className="mt-6 pt-4 border-t border-white/[0.06] text-center"><p className="text-xs text-[#fafaf9]/40">Handles multiple requests in one conversation</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-semibold text-center mb-12">What makes our AI different</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: MessageSquare, title: 'Contextual Memory', desc: 'Remembers everything from earlier in the call.' },
              { icon: Zap, title: 'Real-time Thinking', desc: 'Processes and responds in milliseconds.' },
              { icon: Shield, title: 'Stays On-Brand', desc: 'Never goes off-script or says anything inappropriate.' },
              { icon: Globe, title: 'Industry Knowledge', desc: 'Pre-trained on 12+ industries.' },
              { icon: BrainCircuit, title: 'Emotional Intelligence', desc: 'Detects frustration and adjusts tone.' },
              { icon: Sparkles, title: 'Continuous Learning', desc: 'Gets smarter with every call.' },
            ].map((item) => (
              <div key={item.title} className="p-6 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500/10 mb-4"><item.icon className="h-5 w-5 text-emerald-400" /></div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: 'Voice Options', desc: '16+ premium ElevenLabs voices', href: '/features/voice-options' },
              { title: 'Knowledge Base', desc: 'AI learns from their website', href: '/features/knowledge-base' },
              { title: 'Industry Training', desc: '12+ industry presets', href: '/features/industries' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Give your clients an AI that actually thinks</h2>
          <p className="mt-4 text-[#fafaf9]/50">GPT-4 intelligence included in all plans.</p>
