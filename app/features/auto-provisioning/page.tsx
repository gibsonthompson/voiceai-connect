...
import { ArrowRight, Check, ChevronRight, Menu, X, Zap, Clock, Settings, Globe } from 'lucide-react';
...
export default function AutoProvisioningFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Auto Provisioning</span></div></div></div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">60-second setup.<span className="block mt-2 text-[#fafaf9]/40">Client live in under a minute.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60">Enter business name, website, and phone number. Click &quot;Create.&quot; Phone number provisioned, AI trained, and ready to take calls in under 60 seconds.</p>
              <div className="mt-8 space-y-3">
                {['Automatic phone number provisioning', 'AI trained from website instantly', 'Zero manual configuration', 'Client can start immediately'].map((item) => (<div key={item} className="flex items-center gap-3"><div className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500/10"><Check className="h-3 w-3 text-emerald-400" /></div><span className="text-[#fafaf9]/70">{item}</span></div>))}
              </div>
            </div>
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <h3 className="font-medium mb-4">New Client Setup</h3>
                <div className="space-y-4">
                  <div><label className="text-xs text-[#fafaf9]/40 mb-1 block">Business Name</label><div className="p-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">Smith Plumbing</div></div>
                  <div><label className="text-xs text-[#fafaf9]/40 mb-1 block">Website</label><div className="p-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">smithplumbing.com</div></div>
                  <div><label className="text-xs text-[#fafaf9]/40 mb-1 block">Area Code</label><div className="p-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">404 (Atlanta)</div></div>
                </div>
                <button className="w-full mt-6 py-3 rounded-lg bg-emerald-500 text-[#050505] font-medium">Create Client →</button>
                <p className="text-center text-xs text-[#fafaf9]/40 mt-3">⚡ Live in under 60 seconds</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-semibold text-center mb-12">What happens automatically</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Clock, title: 'Phone Provisioned', desc: 'Local number in their area code, live instantly.' },
              { icon: Globe, title: 'Website Scraped', desc: 'AI learns services, hours, pricing from site.' },
              { icon: Settings, title: 'AI Configured', desc: 'Industry template applied, greeting set up.' },
              { icon: Zap, title: 'Ready to Go', desc: 'Client can start forwarding calls immediately.' },
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[{ title: 'Knowledge Base', desc: 'AI learns from website', href: '/features/knowledge-base' },{ title: 'Phone Numbers', desc: 'Instant number provisioning', href: '/features/phone-numbers' },{ title: 'Client CRM', desc: 'Manage all clients', href: '/features/client-crm' }].map((item) => (<Link key={item.title} href={item.href} className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-emerald-500/30 transition-colors group"><h3 className="font-medium group-hover:text-emerald-400">{item.title}</h3><p className="text-sm text-[#fafaf9]/50 mt-1">{item.desc}</p></Link>))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Sign a client. Go live in 60 seconds.</h2>
          <p className="mt-4 text-[#fafaf9]/50">Auto provisioning included in all plans.</p>
