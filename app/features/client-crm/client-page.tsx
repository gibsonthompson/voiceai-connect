...
import { ArrowRight, Check, ChevronRight, Menu, X, Users, UserPlus, Settings, BarChart3 } from 'lucide-react';
...
export default function ClientCrmFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Client CRM</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><Users className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">All your clients.<span className="block mt-2 text-[#fafaf9]/40">One dashboard.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Manage every client from a single interface. View their call logs, adjust settings, pause services, or check payment status. Everything you need to run your AI receptionist business.</p>
              <div className="mt-8 space-y-3">
                {['All clients in one place', 'Individual settings per client', 'Payment & subscription status', 'Quick actions and bulk operations'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button className="flex items-center gap-1 text-xs text-emerald-400"><UserPlus className="h-3 w-3" />Add New</button>
                </div>
                <div className="space-y-3">
                  {[
                    { name: 'Smith Plumbing', calls: 47, status: 'Active', mrr: '$199' },
                    { name: 'Johnson Law Firm', calls: 31, status: 'Active', mrr: '$299' },
                    { name: 'Bella Salon', calls: 82, status: 'Active', mrr: '$149' },
                    { name: 'Metro HVAC', calls: 0, status: 'Pending', mrr: '$199' },
                  ].map((client) => (
                    <div key={client.name} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03] hover:bg-white/[0.05] cursor-pointer transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center text-xs">{client.name[0]}</div>
                        <div>
                          <p className="text-sm font-medium">{client.name}</p>
                          <p className="text-xs text-[#fafaf9]/40">{client.calls} calls this month</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-emerald-400">{client.mrr}</p>
                        <span className={`text-xs ${client.status === 'Active' ? 'text-emerald-400' : 'text-amber-400'}`}>{client.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="mt-4 pt-4 border-t border-white/[0.06] text-center">
                  <p className="text-sm text-[#fafaf9]/40">4 clients • <span className="text-emerald-400">$846/mo MRR</span></p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: 'Auto Provisioning', desc: '60-second setup', href: '/features/auto-provisioning' },
              { title: 'Stripe Connect', desc: 'Automated billing', href: '/features/stripe-connect' },
              { title: 'Analytics', desc: 'Per-client metrics', href: '/features/analytics' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Manage clients without the chaos</h2>
          <p className="mt-4 text-[#fafaf9]/50">Client CRM included in all plans.</p>
