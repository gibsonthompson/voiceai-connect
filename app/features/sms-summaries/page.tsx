...
import { ArrowRight, Check, ChevronRight, Menu, X, MessageSquare, Zap, Clock, Bell } from 'lucide-react';
...
export default function SmsSummariesFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">SMS Summaries</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><MessageSquare className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Instant SMS after every call.<span className="block mt-2 text-[#fafaf9]/40">Know what happened in 3 seconds.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60">The moment a call ends, your client gets a text with caller name, phone number, reason for call, and urgency level. No app needed.</p>
              <div className="mt-8 space-y-3">
                {['Delivered in under 3 seconds', 'Includes caller info & reason', 'Urgency level badge', 'Works on any phone'].map((item) => (<div key={item} className="flex items-center gap-3"><div className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500/10"><Check className="h-3 w-3 text-emerald-400" /></div><span className="text-[#fafaf9]/70">{item}</span></div>))}
              </div>
            </div>
            <div className="relative flex justify-center">
              <div className="absolute -inset-4 bg-gradient-to-r from-emerald-500/10 to-transparent rounded-3xl blur-2xl" />
              <div className="relative w-72 rounded-[2.5rem] border-4 border-white/10 bg-[#0a0a0a] p-2 shadow-2xl">
                <div className="rounded-[2rem] bg-[#111] p-4 min-h-[400px]">
                  <div className="flex items-center justify-center gap-1 mb-6"><div className="w-16 h-1 bg-white/20 rounded-full" /></div>
                  <div className="space-y-3">
                    <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                      <p className="text-xs text-[#fafaf9]/40 mb-1">VoiceAI Connect</p>
                      <p className="text-sm font-medium mb-2">📞 New Call Summary</p>
                      <div className="space-y-1 text-sm">
                        <p><span className="text-[#fafaf9]/50">Caller:</span> Sarah Johnson</p>
                        <p><span className="text-[#fafaf9]/50">Phone:</span> (555) 123-4567</p>
                        <p><span className="text-[#fafaf9]/50">Reason:</span> Leaky faucet, wants appointment</p>
                        <p className="mt-2"><span className="px-2 py-0.5 rounded text-xs bg-amber-500/20 text-amber-400">⚡ Important</span></p>
                      </div>
                    </div>
                    <p className="text-center text-xs text-[#fafaf9]/30">Delivered 3 seconds ago</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-semibold text-center mb-12">What&apos;s included in each SMS</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: MessageSquare, title: 'Caller Name', desc: 'Who called (if provided)' },
              { icon: Bell, title: 'Phone Number', desc: 'Click to call back instantly' },
              { icon: Clock, title: 'Call Reason', desc: 'What they needed' },
              { icon: Zap, title: 'Urgency Level', desc: 'Normal, Important, or Urgent' },
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[{ title: 'Urgency Detection', desc: 'AI classifies call priority', href: '/features/urgency-detection' },{ title: 'AI Summaries', desc: 'Detailed call analysis', href: '/features/ai-summaries' },{ title: 'Notifications', desc: 'Email & webhook alerts', href: '/features/notifications' }].map((item) => (<Link key={item.title} href={item.href} className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-emerald-500/30 transition-colors group"><h3 className="font-medium group-hover:text-emerald-400">{item.title}</h3><p className="text-sm text-[#fafaf9]/50 mt-1">{item.desc}</p></Link>))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Stay informed without an app</h2>
          <p className="mt-4 text-[#fafaf9]/50">Unlimited SMS summaries included in all plans.</p>
