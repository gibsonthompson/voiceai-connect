...
import { ArrowRight, Check, ChevronRight, Menu, X, Bell, MessageSquare, Mail, Smartphone } from 'lucide-react';
...
export default function NotificationsFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Notifications</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-6"><Bell className="h-4 w-4 text-emerald-400" /><span className="text-emerald-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Real-time notifications.<span className="block mt-2 text-[#fafaf9]/40">SMS, email, or both.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Your clients get notified instantly after every call. Configure multiple phone numbers, emails, or webhook integrations. They choose how they want to stay informed.</p>
              <div className="mt-8 space-y-3">
                {['Instant SMS notifications', 'Email summaries', 'Multiple recipients', 'Webhook integrations'].map((item) => (
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                    <div className="flex items-center gap-3"><MessageSquare className="h-5 w-5 text-emerald-400" /><div><p className="font-medium">SMS</p><p className="text-xs text-[#fafaf9]/40">(555) 123-4567</p></div></div>
                    <span className="px-2 py-1 rounded bg-emerald-500/20 text-emerald-400 text-xs">Active</span>
                  </div>
                  <div className="flex items-center justify-between p-4 rounded-lg bg-white/[0.03]">
                    <div className="flex items-center gap-3"><Mail className="h-5 w-5 text-[#fafaf9]/40" /><div><p className="font-medium">Email</p><p className="text-xs text-[#fafaf9]/40">owner@business.com</p></div></div>
                    <span className="px-2 py-1 rounded bg-emerald-500/20 text-emerald-400 text-xs">Active</span>
                  </div>
                  <div className="flex items-center justify-between p-4 rounded-lg bg-white/[0.03]">
                    <div className="flex items-center gap-3"><Smartphone className="h-5 w-5 text-[#fafaf9]/40" /><div><p className="font-medium">SMS #2</p><p className="text-xs text-[#fafaf9]/40">(555) 987-6543</p></div></div>
                    <span className="px-2 py-1 rounded bg-white/[0.05] text-[#fafaf9]/40 text-xs">Optional</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: 'SMS Summaries', desc: 'Detailed call summaries via text', href: '/features/sms-summaries' },
              { title: 'Urgency Detection', desc: 'Priority alerts for emergencies', href: '/features/urgency-detection' },
              { title: 'API Access', desc: 'Webhook integrations', href: '/features/api-access' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Never miss an important call</h2>
          <p className="mt-4 text-[#fafaf9]/50">Real-time notifications included in all plans.</p>
