...
import { ArrowRight, Check, ChevronRight, Menu, X, FileText, Search, Download, Clock } from 'lucide-react';
...
export default function TranscriptsFeaturePage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafaf9] overflow-hidden">
      ...
      <div className="pt-24 sm:pt-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 text-sm text-[#fafaf9]/40"><Link href="/features" className="hover:text-[#fafaf9]">Features</Link><ChevronRight className="h-4 w-4" /><span className="text-[#fafaf9]/60">Transcripts</span></div></div></div>

      <section className="relative pt-8 sm:pt-12 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-amber-500/20 bg-amber-500/[0.08] px-4 py-1.5 text-sm mb-6"><FileText className="h-4 w-4 text-amber-400" /><span className="text-amber-300/90">All Plans</span></div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.1]">Word-for-word transcripts.<span className="block mt-2 text-[#fafaf9]/40">99% accuracy.</span></h1>
              <p className="mt-6 text-lg sm:text-xl text-[#fafaf9]/60 leading-relaxed">Every call is automatically transcribed with near-perfect accuracy. Search, export, or review any conversation in seconds.</p>
              <div className="mt-8 space-y-3">
                {['Automatic transcription for every call', 'Full-text search across all calls', 'Export as TXT or PDF', 'Speaker identification (AI vs. caller)'].map((item) => (
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-[#fafaf9]/40">2:34 duration</span>
                </div>
                <div className="space-y-4 text-sm">
                  <div><span className="text-amber-400 font-medium">AI:</span> <span className="text-[#fafaf9]/70">&quot;Good afternoon, Smith Plumbing, how can I help you today?&quot;</span><span className="text-[#fafaf9]/30 text-xs ml-2">0:02</span></div>
                  <div><span className="text-emerald-400 font-medium">Caller:</span> <span className="text-[#fafaf9]/70">&quot;Hi, I have a leaky faucet in my kitchen. It&apos;s been dripping for a few days now.&quot;</span><span className="text-[#fafaf9]/30 text-xs ml-2">0:08</span></div>
                  <div><span className="text-amber-400 font-medium">AI:</span> <span className="text-[#fafaf9]/70">&quot;I&apos;d be happy to help with that. Can I get your name and address to schedule a technician?&quot;</span><span className="text-[#fafaf9]/30 text-xs ml-2">0:15</span></div>
                  <div><span className="text-emerald-400 font-medium">Caller:</span> <span className="text-[#fafaf9]/70">&quot;Sure, it&apos;s Sarah Johnson, 123 Main Street.&quot;</span><span className="text-[#fafaf9]/30 text-xs ml-2">0:22</span></div>
                </div>
                <div className="flex gap-3 mt-6 pt-4 border-t border-white/[0.06]">
                  <button className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-sm"><Search className="h-4 w-4" />Search</button>
                  <button className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-sm"><Download className="h-4 w-4" />Export</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-semibold text-center mb-12">Why transcripts matter</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Search, title: 'Searchable History', desc: 'Find any conversation by keyword, caller name, or topic.' },
              { icon: FileText, title: 'Documentation', desc: 'Perfect records for compliance, disputes, or training.' },
              { icon: Download, title: 'Easy Export', desc: 'Download transcripts for CRM, reports, or archives.' },
              { icon: Clock, title: 'Time Stamps', desc: 'Jump to any moment in the call with timestamps.' },
            ].map((item) => (
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: 'Call Recordings', desc: 'Audio playback of every call', href: '/features/call-recordings' },
              { title: 'AI Summaries', desc: 'Intelligent call highlights', href: '/features/ai-summaries' },
      ...
      <section className="py-16 border-t border-white/[0.06]">
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="text-2xl font-semibold">Every word captured. Every call searchable.</h2>
          <p className="mt-4 text-[#fafaf9]/50">Full transcripts included in all plans.</p>
