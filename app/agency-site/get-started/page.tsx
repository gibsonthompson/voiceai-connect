...
import { 
  ...
  ArrowLeft, Sparkles, Shield, Clock, Zap, Globe, CheckCircle2,
...
// ============================================================================
// PHASE 2B: Generate temporary password
// Same pattern as add-client page — 8 chars, mix of letters/numbers
// ============================================================================
...
function ThemedFormInput({
  ...
  icon: Icon, className = '', isDark = true, primaryColor = '#10b981', maxLength,
}: {
  label: string; name: string; type?: string; placeholder: string; value: string;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required?: boolean;
  icon?: React.ComponentType<{ className?: string }>; className?: string;
  isDark?: boolean; primaryColor?: string; maxLength?: number;
}) {
  return (
...
function ThemedSelect({
  label, name, value, onChange, required = false, icon: Icon, children, isDark = true, primaryColor = '#10b981',
}: {
...
// PHASE 2B: Single-step signup with temp password — no plan selection redirect
// ============================================================================
function ClientSignupForm({ agency }: { agency: Agency }) {
  ...
  // PHASE 2B: Loading phase messages
  ...
  // ============================================================================
  // PHASE 2B: Submit directly to API with temp password — no plan selection
  // ============================================================================
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoadingPhase('Creating your AI receptionist...');
      ...
      const response = await fetch(`${backendUrl}/api/client/signup`, {
        ...
        body: JSON.stringify({
          ...
          agencyId: agency.id,
          password: tempPassword,  // PHASE 2B: Send temp password
        }),
      });
  ...
  const renderStateField = () => {
    ...
    return (
      <ThemedFormInput label="State / Region" name="state" placeholder="Region" value={formData.state}
        onChange={handleChange} required isDark={isDark} primaryColor={primaryColor} />
    );
  };

  // ==========================================================================
  // PHASE 2B: SUCCESS SCREEN — shows credentials after signup
  // ==========================================================================
          ...
          <div className="relative mx-auto max-w-lg">
            <div className="rounded-2xl sm:rounded-3xl backdrop-blur-xl p-6 sm:p-8 shadow-2xl"
              ...
              <div className="text-center mb-6">
                ...
                <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm mb-3"
                ...
                <p className="mt-2" style={{ color: mutedTextColor }}>
                  Your AI receptionist for {credentials.businessName} is ready.
                </p>
              </div>

              {/* AI Phone Number */}
              {credentials.phoneNumber && (
                <div className="mb-5 rounded-xl p-4" style={{ backgroundColor: `${primaryColor}08`, border: `1px solid ${primaryColor}25` }}>
                  <p className="text-xs mb-1.5" style={{ color: mutedTextColor }}>Your AI Phone Number</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <Phone className="h-5 w-5" style={{ color: primaryColor }} />
                      <span className="text-xl font-bold font-mono" style={{ color: primaryColor }}>
                        {formatPhoneDisplay(credentials.phoneNumber)}
                      </span>
                    </div>
                    <button onClick={() => handleCopy(credentials.phoneNumber, 'phone')}
                      className="p-2 rounded-lg transition-colors" style={{ color: mutedTextColor }}>
                      {copiedField === 'phone' ? <Check className="h-4 w-4" style={{ color: primaryColor }} /> : <Copy className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
              )}
              ...
              {/* Login Credentials */}
              <div className="mb-5 rounded-xl p-4" style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.03)' : '#f9fafb', border: `1px solid ${cardBorder}` }}>
                <p className="text-xs font-medium mb-3" style={{ color: mutedTextColor }}>
                ...
                <div className="flex items-center justify-between mb-3 pb-3" style={{ borderBottom: `1px solid ${cardBorder}` }}>
                  ...
                  <button onClick={() => handleCopy(credentials.email, 'email')}
                    className="p-2 rounded-lg transition-colors" style={{ color: mutedTextColor }}>
                    {copiedField === 'email' ? <Check className="h-4 w-4" style={{ color: primaryColor }} /> : <Copy className="h-4 w-4" />}
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs" style={{ color: mutedTextColor }}>Temporary Password</p>
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium font-mono">
                        {showPassword ? credentials.password : '••••••••'}
                      </p>
                      <button onClick={() => setShowPassword(!showPassword)}
                        className="p-1 rounded transition-colors" style={{ color: mutedTextColor }}>
                  ...
                  <button onClick={() => handleCopy(credentials.password, 'password')}
                    className="p-2 rounded-lg transition-colors" style={{ color: mutedTextColor }}>
                    {copiedField === 'password' ? <Check className="h-4 w-4" style={{ color: primaryColor }} /> : <Copy className="h-4 w-4" />}
                  </button>
                </div>
              </div>

              {/* Warning */}
              <div className="mb-6 rounded-xl p-3 text-center" style={{
                ...
                <p className="text-xs" style={{ color: isDark ? '#fbbf24' : '#92400e' }}>
                  Save your password now — you won&apos;t see it again. Check your email for a backup link.
                </p>
              </div>

              {/* CTA Button */}
              <a
                href="/client/login?signup=success"
        ...
        <div className="relative mx-auto max-w-lg">
          <div className="rounded-2xl sm:rounded-3xl backdrop-blur-xl p-6 sm:p-8 shadow-2xl"
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-5">
              <ThemedFormInput label="Business Name" name="businessName" placeholder="Acme Plumbing Services"
                value={formData.businessName} onChange={handleChange} required icon={Building} isDark={isDark} primaryColor={primaryColor} />
              <ThemedFormInput label="Your Name" name="ownerName" placeholder="John Smith"
                value={formData.ownerName} onChange={handleChange} required icon={User} isDark={isDark} primaryColor={primaryColor} />
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <ThemedFormInput label="Email" name="email" type="email" placeholder="you@business.com"
                  value={formData.email} onChange={handleChange} required icon={Mail} isDark={isDark} primaryColor={primaryColor} />
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <ThemedFormInput label="City" name="city" placeholder={formData.country === 'GB' ? 'London' : formData.country === 'AU' ? 'Sydney' : 'Atlanta'}
                  value={formData.city} onChange={handleChange} required icon={MapPin} isDark={isDark} primaryColor={primaryColor} />
                {renderStateField()}
              </div>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? (
                  <>
                    ...
                    {loadingPhase || 'Setting up your account...'}
                  </>
                ) : (
                  <>
                    Start Free Trial
...
// ============================================================================
// AGENCY SIGNUP FORM — UNCHANGED from original
// ============================================================================
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/agency/signup`, {
        ...
        body: JSON.stringify({
          ...
          country: formData.country, referralCode: referralCode,
        }),
      });
      const data = await response.json();
        ...
        <div className="relative mx-auto max-w-md">
          ...
          <div className="rounded-2xl sm:rounded-3xl border border-white/[0.08] bg-[#0a0a0a]/50 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-black/20">
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-[#fafaf9]/70">Country</label>
                <div className="relative">
              ...
              <button type="submit" disabled={loading || googleLoading}
                ...
                {loading ? (
                  ...
                  <><Mail className="h-5 w-5 text-[#fafaf9]/60" /> Continue with Email <ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" /></>
                )}
              </button>
            </form>
