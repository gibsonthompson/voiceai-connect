...
function ProgressSteps({ currentStep, totalSteps = 3, accentColor = '#10b981' }: { currentStep: number; totalSteps?: number; accentColor?: string }) {
  return (
    <div className="flex items-center justify-center gap-2">
      {Array.from({ length: totalSteps }, (_, i) => i + 1).map((step) => (
        <div key={step} className="flex items-center gap-2">
          <div className={`h-2 rounded-full transition-all duration-300 ${step === currentStep ? 'w-8' : 'w-2'}`}
            style={{ backgroundColor: step <= currentStep ? accentColor : 'rgba(255,255,255,0.1)' }} />
        </div>
      ))}
    </div>
  );
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {[...Array(20)].map((_, i) => (
        <div key={i} className="absolute w-2 h-2 rounded-full animate-confetti"
          style={{ left: `${Math.random() * 100}%`, top: '-10px', backgroundColor: ['#10b981', '#f59e0b', '#3b82f6', '#ec4899', '#8b5cf6'][i % 5], animationDelay: `${Math.random() * 2}s`, animationDuration: `${2 + Math.random() * 2}s` }} />
      ))}
    </div>
  );
}
...
function ClientSuccessPage({ agency, isEmbed }: { agency: Agency | null; isEmbed: boolean }) {
  ...
  const nextSteps = [
    { icon: MessageSquare, title: 'Check your phone', description: 'We sent you a text with login instructions' },
    { icon: PhoneForwarded, title: 'Forward your calls', description: 'Set up call forwarding to your AI number' },
      ...
      <main className={mainPaddingClass}>
        <div className="relative w-full max-w-lg">
          ...
          <div className="rounded-2xl sm:rounded-3xl border border-white/[0.08] bg-[#0a0a0a]/50 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-black/20 text-center">
            <div className="mx-auto w-16 h-16 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center mb-6 relative" style={{ backgroundColor: `${accentColor}15` }}>
            ...
            <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm mb-4" style={{ backgroundColor: `${primaryColor}15`, border: `1px solid ${primaryColor}30` }}>
            ...
            <p className="text-[#fafaf9]/50 mb-6 sm:mb-8">{businessName ? `Your AI receptionist for ${businessName} is ready to take calls.` : 'Your AI receptionist is ready to take calls.'}</p>
            {phoneNumber && (
              <div className="mb-6 sm:mb-8">
                <p className="text-sm text-[#fafaf9]/50 mb-3">Your AI Phone Number</p>
                <div className="inline-flex items-center gap-3 rounded-xl px-5 sm:px-6 py-4 border" style={{ backgroundColor: `${primaryColor}10`, borderColor: `${primaryColor}30` }}>
                  <Phone className="h-5 w-5" style={{ color: primaryColor }} />
                  <span className="text-xl sm:text-2xl font-bold font-mono tracking-wide" style={{ color: primaryColor }}>{phoneNumber}</span>
                  <button onClick={handleCopyPhone} className="p-2 rounded-lg hover:bg-white/10 transition-colors ml-1" title="Copy phone number">
                    {copied ? <Check className="h-5 w-5 text-emerald-400" /> : <Copy className="h-5 w-5 text-[#fafaf9]/40 hover:text-[#fafaf9]/70" />}
                  </button>
                </div>
                {copied && <p className="text-xs text-emerald-400 mt-2 animate-fade-in">Copied to clipboard!</p>}
              </div>
            )}
            <div className="text-left mb-6 sm:mb-8">
              ...
              <div className="space-y-3">
                {nextSteps.map((step, i) => (
                  <div key={i} className="flex items-start gap-4 p-3 rounded-xl bg-white/[0.02] border border-white/[0.06]">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: `${accentColor}10` }}>
                      <step.icon className="h-5 w-5" style={{ color: accentColor }} />
                    </div>
                    <div><p className="font-medium text-sm">{step.title}</p><p className="text-sm text-[#fafaf9]/50">{step.description}</p></div>
                  </div>
                ))}
              </div>
            </div>
            <Link href="/client/login" className="group w-full inline-flex items-center justify-center gap-2 rounded-full px-6 py-4 text-base font-medium transition-all hover:scale-[1.02] hover:shadow-lg active:scale-[0.98]" style={{ backgroundColor: primaryColor, color: primaryLight ? '#050505' : '#fafaf9', boxShadow: `0 0 40px ${primaryColor}30` }}>
              Go to Dashboard<ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" />
            </Link>
          </div>
          <p className="mt-6 text-center text-sm text-[#fafaf9]/40">Need help? Contact <span className="text-[#fafaf9]/60">{agency?.name || 'support'}</span> for assistance.</p>
        </div>
      </main>
...
function AgencySuccessPage({ isEmbed }: { isEmbed: boolean }) {
  ...
  const getAgencySlug = (): string | null => {
    try {
      const stored = localStorage.getItem('agency');
      if (stored) { const data = JSON.parse(stored); return data.slug || null; }
    } catch {}
    return null;
  };
  ...
  const handleCopyLink = () => { if (signupLink) { navigator.clipboard.writeText(signupLink); setCopiedLink(true); setTimeout(() => setCopiedLink(false), 2000); } };

  const nextSteps = [
    { icon: LayoutDashboard, title: 'Explore your dashboard', description: 'See your clients, analytics, and settings all in one place', href: '/agency/dashboard' },
    { icon: CreditCard, title: isFree ? 'Add a payment method' : 'Connect Stripe', description: isFree ? 'Required before adding your first client' : 'Start receiving client payments directly to your account', href: '/agency/settings?tab=payments' },
    { icon: Users, title: 'Add your first client', description: 'Onboard a client or share your signup link', href: '/agency/clients' },
        ...
        <div className="relative w-full max-w-lg">
          ...
          <div className="rounded-2xl sm:rounded-3xl border border-white/[0.08] bg-[#0a0a0a]/50 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-black/20 text-center">
            ...
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-4 py-1.5 text-sm mb-4">
              {isFree ? <Zap className="h-4 w-4 text-emerald-400" /> : <Rocket className="h-4 w-4 text-emerald-400" />}
              <span className="text-emerald-300/90">{isFree ? 'Your free account is active' : 'Your trial is active'}</span>
            </div>

            <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight mb-2">Welcome to VoiceAI Connect!</h1>
            <p className="text-[#fafaf9]/50 mb-6 sm:mb-8">
              {isFree
                ? 'Your free account is ready. Add a payment method and start onboarding clients.'
            ...
            {signupLink && (
              <div className="mb-6 sm:mb-8">
                ...
                <div className="flex items-center gap-2 rounded-xl px-4 py-3 border border-emerald-500/20 bg-emerald-500/[0.05]">
                  ...
                  <button onClick={handleCopyLink} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors flex-shrink-0" title="Copy link">
                    {copiedLink ? <Check className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4 text-[#fafaf9]/40 hover:text-[#fafaf9]/70" />}
                  </button>
                </div>
                {copiedLink && <p className="text-xs text-emerald-400 mt-1.5">Copied!</p>}
              </div>
            )}

            <div className="text-left mb-6 sm:mb-8">
              <h3 className="font-medium mb-4 text-center sm:text-left">{isFree ? 'Get started' : 'Get started in 4 steps'}</h3>
              <div className="space-y-3">
                {nextSteps.map((step, i) => (
                  <a key={i} href={step.href} className="flex items-start gap-4 p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:bg-white/[0.04] hover:border-white/[0.1] transition-all group">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[#fafaf9]/30 bg-white/[0.05] px-2 py-0.5 rounded-full">Step {i + 1}</span>
                      </div>
            ...
            <Link href="/agency/dashboard" className="group w-full inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-4 text-base font-medium text-[#050505] transition-all hover:bg-[#fafaf9] hover:scale-[1.02] hover:shadow-xl hover:shadow-white/10 active:scale-[0.98]">
              Go to Dashboard<ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" />
            </Link>
          </div>

          <div className="mt-6 text-center">
            <p className="text-sm text-[#fafaf9]/40">Questions? Reach out anytime from <a href="/agency/settings?tab=feedback" className="text-emerald-400 hover:text-emerald-300 underline underline-offset-2">Settings → Feedback</a></p>
          </div>
        </div>
      </main>
