...
function AgencySettingsContent() {
  ...
  const [customFeatures, setCustomFeatures] = useState<{ key: string; label: string }[]>([]);
  const [featureOverrides, setFeatureOverrides] = useState<Record<string, { label?: string; hidden?: boolean }>>({});
  ...
  const canEnableCardRequired = clientBillingMode !== 'manual' && !!(agency?.stripe_account_id && (agency as any)?.stripe_charges_enabled);
  ...
  const resetPlanFeatures = () => { setPlanFeatures(DEFAULT_PLAN_FEATURES); setFeatureOverrides({}); };
  const slugifyFeatureKey = (label: string) => 'cf_' + (label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 24) || 'feature') + '_' + Math.random().toString(36).slice(2, 5);
  const addCustomFeature = () => { const label = newFeatureLabel.trim(); if (!label || customFeatures.length >= 15) return; setCustomFeatures((prev) => [...prev, { key: slugifyFeatureKey(label), label }]); setNewFeatureLabel(''); };
  const renameCustomFeature = (key: string, label: string) => setCustomFeatures((prev) => prev.map((f) => (f.key === key ? { ...f, label } : f)));
  const deleteCustomFeature = (key: string) => { setCustomFeatures((prev) => prev.filter((f) => f.key !== key)); setPlans((prev) => prev.map((pl) => { const rest = { ...pl.features }; delete rest[key]; return { ...pl, features: rest }; })); };
  const renameBuiltin = (key: string, label: string) => setFeatureOverrides((prev) => ({ ...prev, [key]: { ...prev[key], label } }));
  const hideBuiltin = (key: string) => setFeatureOverrides((prev) => ({ ...prev, [key]: { ...prev[key], hidden: true } }));
  const restoreBuiltin = (key: string) => setFeatureOverrides((prev) => { const cur = prev[key]; if (!cur) return prev; const next = { ...prev }; if (cur.label) next[key] = { label: cur.label }; else delete next[key]; return next; });
  const builtinLabel = (k: string) => featureOverrides[k]?.label || FEATURE_LABELS[k]?.label || k;
  ...
  const handleTogglePlanChanges = async () => {
    if (!agency) return;
    const next = !allowClientPlanChanges;
    setPlanChangesToggleSaving(true);
    setAllowClientPlanChanges(next); // optimistic
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${backendUrl}/api/agency/${agency.id}/settings`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ allow_client_plan_changes: next }),
      });
      if (!res.ok) setAllowClientPlanChanges(!next);
    } catch (e) { setAllowClientPlanChanges(!next); }
    finally { setPlanChangesToggleSaving(false); }
  };
  ...
  const saveBillingMethod = async (method: string) => {
    setBillingMethod(method);
    if (!agency?.id) return;
    try {
      const token = localStorage.getItem('auth_token');
      await fetch(`${backendUrl}/api/agency/${agency.id}/settings`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ billing_method: method }),
      });
    } catch { /* record-only; non-fatal */ }
  };
  ...
  return (
    <div className="agency-settings p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className={`flex-1 ${activeTab === 'pricing' ? 'max-w-5xl' : 'max-w-2xl'}`}>
            ...
            {activeTab === 'pricing' && (
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="rounded-xl p-3 sm:p-4" style={{ backgroundColor: theme.input, border: `1px solid ${theme.inputBorder}` }}>
                  <div className="flex items-center gap-2 mb-1"><Sparkles className="h-4 w-4" style={{ color: theme.primary }} /><h4 className="font-medium text-sm sm:text-base">Features</h4></div>
                  <p className="text-[11px] sm:text-xs mb-3" style={{ color: theme.textMuted }}>Rename or remove any feature, or add your own. They show up as toggle rows in the grid above, so you include or exclude them per plan.</p>
                  <div className="space-y-2 mb-3">
                    {FEATURE_ORDER.filter((k) => !featureOverrides[k]?.hidden).map((k) => (
                      <div key={k} className="flex items-center gap-2"><input value={builtinLabel(k)} onChange={(e) => renameBuiltin(k, e.target.value)} maxLength={60} className="flex-1 min-w-0 rounded-lg px-3 py-2 text-sm" style={{ backgroundColor: theme.isDark ? '#050505' : '#f9fafb', border: `1px solid ${theme.inputBorder}`, color: theme.text }} /><button type="button" onClick={() => hideBuiltin(k)} title="Remove feature" className="p-2 rounded-lg flex-shrink-0" style={{ color: theme.textMuted }}><Trash2 className="h-4 w-4" /></button></div>
                    ))}
                    {customFeatures.map((cf) => (
                      <div key={cf.key} className="flex items-center gap-2"><input value={cf.label} onChange={(e) => renameCustomFeature(cf.key, e.target.value)} maxLength={60} className="flex-1 min-w-0 rounded-lg px-3 py-2 text-sm" style={{ backgroundColor: theme.isDark ? '#050505' : '#f9fafb', border: `1px solid ${theme.inputBorder}`, color: theme.text }} /><button type="button" onClick={() => deleteCustomFeature(cf.key)} title="Delete feature" className="p-2 rounded-lg flex-shrink-0" style={{ color: theme.textMuted }}><Trash2 className="h-4 w-4" /></button></div>
                    ))}
                  </div>
                  {FEATURE_ORDER.some((k) => featureOverrides[k]?.hidden) && (
                    <div className="flex flex-wrap items-center gap-1.5 mb-3">
                      <span className="text-[11px]" style={{ color: theme.textMuted }}>Removed:</span>
                      {FEATURE_ORDER.filter((k) => featureOverrides[k]?.hidden).map((k) => (
                        <button key={k} type="button" onClick={() => restoreBuiltin(k)} title="Restore this feature" className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full" style={{ backgroundColor: theme.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.04)', color: theme.textMuted }}><Plus className="h-3 w-3" />{FEATURE_LABELS[k]?.label || k}</button>
                      ))}
                    </div>
                  )}
                  <div className="flex items-center gap-2"><input value={newFeatureLabel} onChange={(e) => setNewFeatureLabel(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addCustomFeature(); } }} placeholder="Add a feature, e.g. Dedicated account manager" maxLength={60} className="flex-1 min-w-0 rounded-lg px-3 py-2 text-sm" style={{ backgroundColor: theme.isDark ? '#050505' : '#f9fafb', border: `1px solid ${theme.inputBorder}`, color: theme.text }} /><button type="button" onClick={addCustomFeature} disabled={!newFeatureLabel.trim() || customFeatures.length >= 15} className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium flex-shrink-0 disabled:opacity-40" style={{ backgroundColor: theme.primary, color: theme.primaryText }}><Plus className="h-4 w-4" /> Add</button></div>
                ...
                {/* Client Per-Minute Billing. Optional: charge your OWN clients
                    ...
                    existing clients. */}
                {clientBillingMode === 'manual' && (
                  <div className="mb-3 rounded-xl p-3 flex items-start gap-2.5" style={{ backgroundColor: theme.infoBg, border: `1px solid ${theme.infoBorder}` }}>
                    <Info className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: theme.infoText }} />
                    <p className="text-xs sm:text-sm" style={{ color: theme.textMuted }}>Not needed while you&apos;re billing clients yourself. Per-minute charges run through Stripe Connect, which is off in manual mode.</p>
                  </div>
                )}
