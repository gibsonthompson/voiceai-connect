...
export default function MarketingWebsitePage() {
  ...
  // Live-preview iframe src: the exact URL embed.js injects (see public/embed.js
  // buildIframeSrc), so the on-page preview shows the real signup form, not a
  // mockup. embed=true renders the bare form (no marketing chrome); agency=<id>
  ...
  // Client Login: where agencies send their clients to sign in.
  const loginUrl = agency?.marketing_domain && agency?.domain_verified ? `https://${agency.marketing_domain}/client/login` : `${subdomainUrl}/client/login`;
  const loginButtonSnippet = `<a href="${loginUrl}" style="display:inline-block;padding:12px 28px;background:transparent;color:${agencyPrimaryColor};border:2px solid ${agencyPrimaryColor};border-radius:8px;text-decoration:none;font-weight:600;font-size:15px">Client Login</a>`;
  ...
  useEffect(() => {
    if (demoMode) { setTagline('AI-Powered Phone Answering'); setHeadline('Never Miss Another Call'); setSubheadline('Our AI receptionist answers calls 24/7.'); setCustomDomain('voiceai.youragency.com'); setDomainStatus('verified'); setPrimaryColor(agency?.primary_color || '#10b981'); setSecondaryColor(agency?.secondary_color || '#059669'); setAccentColor(agency?.accent_color || '#34d399'); setSelectedTemplate(agency?.marketing_template || 'classic'); setCustomNavLinks([{ label: 'Home', url: 'https://yourmainsite.com' }]); setDisplayCurrency(''); setGtmId(''); setFbPixelId(''); setGoogleAnalyticsId(''); setCustomHeadScripts(''); setCustomBodyScripts(''); setOgTitle(''); setOgDescription(''); setOgImageUrl(''); setPlans([]); return; }
    if (agency) { if (agency.marketing_domain) { setCustomDomain(agency.marketing_domain); setDomainStatus(agency.domain_verified ? 'verified' : 'pending'); } setTagline(agency.company_tagline || ''); setHeadline(agency.website_headline || ''); setSubheadline(agency.website_subheadline || ''); setPrimaryColor(agency.primary_color || '#10b981'); setSecondaryColor(agency.secondary_color || '#059669'); setAccentColor(agency.accent_color || '#34d399'); setSelectedTemplate(agency.marketing_template || 'classic'); setCustomNavLinks(Array.isArray((agency as any).custom_nav_links) ? (agency as any).custom_nav_links : []); setDisplayCurrency((agency as any).display_currency || ''); setGtmId(agency.gtm_id || ''); setFbPixelId(agency.fb_pixel_id || ''); setGoogleAnalyticsId(agency.google_analytics_id || ''); setCustomHeadScripts(agency.custom_head_scripts || ''); setCustomBodyScripts(agency.custom_body_scripts || ''); setOgTitle(agency.og_title || ''); setOgDescription(agency.og_description || ''); setOgImageUrl(agency.og_image_url || ''); setPlans(Array.isArray((agency as any).plans) ? (agency as any).plans : []); }
  }, [agency, demoMode]);
  ...
  const handleSaveColors = async () => { if (demoMode) { setColorsSaved(true); setTimeout(() => setColorsSaved(false), 3000); return; } if (!agency) return; setSavingColors(true); setColorsSaved(false); try { const token = localStorage.getItem('auth_token'); const response = await fetch(`${backendUrl}/api/agency/${agency.id}/settings`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ primary_color: primaryColor, secondary_color: secondaryColor, accent_color: accentColor }) }); if (response.ok) { await refreshAgency(); setColorsSaved(true); setTimeout(() => setColorsSaved(false), 3000); } } catch (error) { console.error('Failed to save colors:', error); } finally { setSavingColors(false); } };
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {activeTab === 'overview' && (<div className="space-y-4 sm:space-y-6">
        ...
        {/* ── Your Clients' Dashboard ────────────────────────────────────────
            Prevents the onboarding confusion surfaced by a support request:
            agencies not realizing the client dashboard is a SEPARATE, always-
            hosted destination reached via a login link, independent of whatever
            marketing site (ours, theirs, or none) they use. This card states
            that model up front and shows the three ways a client reaches it, so
            no agency has to ask "how do my clients get to their dashboard?"
        ─────────────────────────────────────────────────────────────────── */}
        <div className="rounded-xl p-4 sm:p-6" style={{ backgroundColor: isDark ? `${agencyPrimaryColor}0d` : `${agencyPrimaryColor}0a`, border: `1px solid ${agencyPrimaryColor}40` }}>
          <div className="flex items-center gap-2 mb-1.5"><LayoutDashboard className="h-4 w-4" style={{ color: agencyPrimaryColor }} /><h3 className="font-medium text-sm sm:text-base" style={{ color: textColor }}>Your clients&apos; dashboard</h3></div>
          <p className="text-xs sm:text-sm mb-4" style={{ color: mutedTextColor }}>Every client you add gets their own dashboard, hosted here and always available at the link below. It&apos;s separate from your marketing page, so it works whether you use our website, your own website, or no website at all.</p>

          <label className="block text-xs font-medium mb-1.5" style={{ color: isDark ? 'rgba(250,250,249,0.7)' : '#374151' }}>Your client login link</label>
        ...
        {/* ── Embed on Your Own Website ─────────────────────────────────────
            Two ways to bring signups onto the agency's existing site.
            (A) Inline widget (recommended): the iframe-based embed.js form,
                shown here with a LIVE preview (a real /signup?embed=true iframe,
                the same URL embed.js injects) so the agency sees the actual
                form, with the paste-in snippet below it.
            (B) Embed button: a simple styled anchor that opens the signup page
                in a new tab. No JS, useful if a prospect's page CSP blocks
                iframes. Keeps its own live button preview.
            Both target the same signup destination (this agency).
        ─────────────────────────────────────────────────────────────────── */}
        <div className="rounded-2xl p-5 sm:p-7" style={{ backgroundColor: cardBg, border: `1px solid ${borderColor}` }}>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <h4 className="text-sm sm:text-base font-semibold" style={{ color: textColor }}>Signup widget</h4>
            ...
            <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${inputBorder}` }}>
              <div className="flex items-center gap-2 px-3 py-2" style={{ backgroundColor: inputBg, borderBottom: `1px solid ${inputBorder}` }}>
                ...
                <span className="flex items-center gap-1.5 ml-1.5 text-[10px] sm:text-xs font-medium" style={{ color: mutedTextColor }}><Eye className="h-3.5 w-3.5" />Live preview, the real form your visitors see</span>
              </div>
              {agency?.id && !demoMode && hasAccess ? (
                <iframe title="Signup widget preview" src={widgetPreviewSrc} loading="lazy" className="block w-full" style={{ height: 560, border: 0, backgroundColor: '#ffffff' }} />
              ) : (
                <div className="flex items-center justify-center text-center px-6" style={{ height: 320, backgroundColor: inputBg }}>
                  <p className="text-xs sm:text-sm" style={{ color: mutedTextColor }}>{demoMode ? 'Turn off demo mode to load your live signup form here.' : 'Your live signup form preview will appear here.'}</p>
                </div>
              )}
            </div>
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-medium" style={{ color: isDark ? 'rgba(250,250,249,0.7)' : '#374151' }}>Add it to your site</label>
                <button onClick={() => copyToClipboard(iframeWidgetSnippet, 'widget-code')} className="text-xs font-medium inline-flex items-center gap-1" style={{ color: agencyPrimaryColor }}>{copied === 'widget-code' ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />} Copy</button>
              </div>
              ...
              <p className="mt-2 text-[10px] sm:text-xs" style={{ color: mutedTextColor }}>Paste it where you want the form to appear. It keeps working even if you change your subdomain.</p>
            </div>
          ...
          {/* ── Divider ── */}
          <div className="my-6" style={{ borderTop: `1px solid ${borderColor}` }} />

          {/* ── Section B: Embed Button (link-style) ── */}
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <h4 className="text-sm sm:text-base font-semibold" style={{ color: textColor }}>Embed button</h4>
            ...
            <p className="text-xs sm:text-sm mb-3" style={{ color: mutedTextColor }}>A simple branded link to your signup page. Opens in a new tab, useful if your page can&apos;t load external scripts.</p>
            ...
            <div className="mt-3 flex items-center gap-2"><span className="text-[10px] sm:text-xs" style={{ color: mutedTextColor }}>Preview:</span><a href={signupUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', padding: '14px 32px', background: agencyPrimaryColor, color: isLightColor(agencyPrimaryColor) ? '#050505' : '#ffffff', borderRadius: '8px', textDecoration: 'none', fontWeight: 600, fontSize: '16px' }}>Start Free Trial</a></div>
          </div>
        </div>
      </div>)}
