      ...
      <div className="w-full max-w-5xl rounded-2xl overflow-hidden flex flex-col" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}`, maxHeight: '90vh' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: theme.border }}>
          <div className="flex items-center gap-3">
            {callState === 'connected' ? (
              <span className="flex items-center gap-1.5 text-sm font-bold px-3 py-1 rounded-full" style={{ backgroundColor: theme.isDark ? 'rgba(34,197,94,0.15)' : '#f0fdf4', color: '#22c55e' }}>
                <Radio className="h-3 w-3 animate-pulse" /> LIVE
              </span>
            ) : callState === 'connecting' ? (
              <span className="flex items-center gap-1.5 text-sm font-bold px-3 py-1 rounded-full" style={{ backgroundColor: theme.primary15, color: theme.primary }}>
                <Loader2 className="h-3 w-3 animate-spin" /> CONNECTING
              </span>
            ) : (
              <span className="text-sm font-bold px-3 py-1 rounded-full" style={{ backgroundColor: theme.hover, color: theme.textMuted }}>ENDED</span>
            )}
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          <div className="flex-1 flex flex-col border-r" style={{ borderColor: theme.border }}>
            <div className="px-4 py-2 border-b" style={{ borderColor: theme.border }}>
              <span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Transcript</span>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {transcript.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full py-8">
                  <Bot className="h-8 w-8 mb-2" style={{ color: theme.textMuted, opacity: 0.15 }} />
                  <p className="text-sm" style={{ color: theme.textMuted }}>Waiting for speech...</p>
                </div>
              )}
              {transcript.map(e => (
          ...
          <div className="w-72 flex-shrink-0 flex-col hidden md:flex">
            <div className="px-4 py-2 border-b flex items-center justify-between" style={{ borderColor: theme.border }}>
              <span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Events</span>
              <span className="text-sm font-mono" style={{ color: theme.textMuted }}>{eventLog.length}</span>
            </div>
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {eventLog.map(e => {
                const c = { info: theme.textMuted, warn: '#f59e0b', error: '#ef4444', success: '#22c55e' }[e.level];
                return (
                  <div key={e.id} className="rounded px-2 py-1 text-sm font-mono" style={{ backgroundColor: theme.isDark ? 'rgba(255,255,255,0.02)' : '#f9fafb' }}>
                    <div className="flex items-center justify-between gap-1">
                      <span className="font-semibold truncate" style={{ color: c }}>{e.type}</span>
                      <span className="flex-shrink-0 text-[11px]" style={{ color: theme.textMuted }}>{new Date(e.timestamp).toLocaleTimeString([], { hour12: false })}</span>
                    </div>
    ...
    return (
      <LockedFeature title="AI Lab" description="Configure, test, and ship AI receptionists with industry templates, voice selection, and live browser calls." requiredPlan="Pro"
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          <div className="flex items-center gap-3 mb-6"><div className="flex h-10 w-10 items-center justify-center rounded-xl" style={{ backgroundColor: theme.primary15 }}><FlaskConical className="h-5 w-5" style={{ color: theme.primary }} /></div><div><h1 className="text-xl sm:text-2xl font-semibold tracking-tight" style={{ color: theme.text }}>AI Lab</h1><p className="text-sm sm:text-sm" style={{ color: theme.textMuted }}>Configure, test, and ship AI receptionists</p></div></div>
          <div className="mb-8"><div className="flex items-center gap-2 mb-2"><Package className="h-4 w-4" style={{ color: theme.primary }} /><span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Packaged Receptionists</span></div><p className="text-sm mb-4" style={{ color: theme.textMuted }}>Configure the default AI receptionist for each industry. New clients inherit your voice, greeting, prompt, and knowledge base.</p>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{PREVIEW_INDUSTRIES.map(ind => { const Ic = ICON_MAP[ind.icon] || Building2; return (<div key={ind.key} className="rounded-xl p-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}><div className="flex items-start justify-between mb-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ backgroundColor: theme.hover }}><Ic className="h-5 w-5" style={{ color: theme.textMuted }} /></div><span className="text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: theme.hover, color: theme.textMuted }}>Default</span></div><p className="font-medium text-sm" style={{ color: theme.text }}>{ind.label}</p><p className="text-sm mt-0.5" style={{ color: theme.textMuted }}>{ind.desc}</p></div>); })}</div></div>
          <div className="flex gap-2 mb-4">{['Acme Plumbing', 'Bright Dental', 'Metro Law'].map(name => (<div key={name} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium" style={{ backgroundColor: theme.card, color: theme.text, border: `1px solid ${theme.border}` }}><div className="w-5 h-5 rounded flex items-center justify-center" style={{ backgroundColor: theme.primary15 }}><Building className="h-3 w-3" style={{ color: theme.primary }} /></div>{name}</div>))}</div>
        </div>
      </LockedFeature>
    );
  }
    ...
    <>
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        {!clientsLoading && clients.length > 0 && (
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3"><div className="relative flex-1 max-w-xs"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5" style={{ color: theme.textMuted }} /><input type="text" value={clientSearch} onChange={e => setClientSearch(e.target.value)} placeholder="Search clients..." className="w-full rounded-lg pl-8 pr-3 py-1.5 text-sm" style={inputStyle} /></div><div className="flex gap-1"><button onClick={() => { if (clientScrollRef.current) clientScrollRef.current.scrollBy({ left: -200, behavior: 'smooth' }); }} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: theme.hover, color: theme.textMuted, border: `1px solid ${theme.border}` }}><ChevronLeft className="h-3.5 w-3.5" /></button><button onClick={() => { if (clientScrollRef.current) clientScrollRef.current.scrollBy({ left: 200, behavior: 'smooth' }); }} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: theme.hover, color: theme.textMuted, border: `1px solid ${theme.border}` }}><ChevronRight className="h-3.5 w-3.5" /></button></div></div>
            <div ref={clientScrollRef} className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}><style>{`.scrollbar-hide::-webkit-scrollbar { display: none; }`}</style>{filtered.map(c => { const isActive = selectedClient?.id === c.id; return (<button key={c.id} onClick={() => selectClient(c)} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium whitespace-nowrap transition-all flex-shrink-0" style={{ backgroundColor: isActive ? theme.primary : theme.card, color: isActive ? theme.primaryText : theme.text, border: `1px solid ${isActive ? theme.primary : theme.border}` }}><div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0" style={{ backgroundColor: isActive ? 'rgba(255,255,255,0.2)' : theme.primary15 }}><Building className="h-3 w-3" style={{ color: isActive ? theme.primaryText : theme.primary }} /></div>{c.business_name}{c.vapi_assistant_id && <CircleDot className="h-2 w-2 flex-shrink-0" style={{ color: isActive ? theme.primaryText : '#22c55e' }} />}</button>); })}</div>
            {filtered.length === 0 && clientSearch.trim() && (<p className="text-sm mt-2" style={{ color: theme.textMuted }}>No clients match &quot;{clientSearch}&quot;</p>)}
          </div>
        )}
        ...
        {!clientsLoading && clients.length === 0 && !selectedClient && !canUseIndustryTemplates && (<div className="text-center py-16"><Building className="h-8 w-8 mx-auto mb-3" style={{ color: theme.textMuted, opacity: 0.3 }} /><p className="text-sm" style={{ color: theme.textMuted }}>No clients yet. Add a client to configure and test their AI receptionist.</p><div className="mt-6 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium" style={{ backgroundColor: theme.hover, color: theme.textMuted, border: `1px solid ${theme.border}` }}><Package className="h-3.5 w-3.5" style={{ color: theme.primary }} /> Industry AI templates are a Scale feature</div></div>)}

        {!selectedClient && !clientsLoading && (
          <div>
            {!canUseIndustryTemplates && (
              <div className="mb-8">
                <LockedFeature requiredPlan="Scale" title="Packaged Receptionists" description="Set the default AI receptionist for each industry so new clients inherit your voice, greeting, prompt, model, and knowledge base automatically." features={['Per-industry AI templates', 'Voice, greeting, prompt & model presets', 'Custom knowledge base per vertical', 'New clients auto-inherit the package']}>
                  <div className="flex items-center gap-2 mb-2"><Package className="h-4 w-4" style={{ color: theme.primary }} /><span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Packaged Receptionists</span><span className="text-sm font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: theme.primary15, color: theme.primary }}>Scale</span></div>
                  <p className="text-sm mb-4" style={{ color: theme.textMuted }}>Configure the default AI receptionist for each industry. When a new client signs up, they inherit your voice, greeting, prompt, model, and knowledge base.</p>
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{INDUSTRIES.map(ind => { const Ic = ICON_MAP[ind.icon] || Building2; return (<div key={ind.value} className="rounded-xl p-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}><div className="flex h-10 w-10 items-center justify-center rounded-lg mb-3" style={{ backgroundColor: theme.hover }}><Ic className="h-5 w-5" style={{ color: theme.textMuted }} /></div><p className="font-medium text-sm" style={{ color: theme.text }}>{ind.label}</p><p className="text-sm mt-0.5" style={{ color: theme.textMuted }}>Preset AI receptionist</p></div>); })}</div>
                </LockedFeature>
              </div>
            )}
            {canUseIndustryTemplates && industries.length > 0 && (
              <div className="mb-8"><div className="flex items-center gap-2 mb-2"><Package className="h-4 w-4" style={{ color: theme.primary }} /><span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Packaged Receptionists</span><span className="text-sm font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: theme.primary15, color: theme.primary }}>Scale</span></div><p className="text-sm mb-4" style={{ color: theme.textMuted }}>Configure the default AI receptionist for each industry. When a new client signs up, they inherit your voice, greeting, prompt, model, and knowledge base.</p>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{industries.map(ind => { const Ic = ICON_MAP[ind.icon] || Building2; if (ind.isCustom) { return (<Link key={ind.frontendKey} href={`/agency/templates/${ind.frontendKey}`} className="rounded-xl p-4 relative transition-all group" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }} onMouseEnter={e => (e.currentTarget.style.borderColor = theme.primary + "60")} onMouseLeave={e => (e.currentTarget.style.borderColor = theme.border)}><button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteIndustry(ind.frontendKey); }} title="Remove custom industry" className="absolute top-3 right-3 p-1 rounded-md z-10" style={{ color: theme.textMuted }}><Trash2 className="h-3.5 w-3.5" /></button><div className="flex h-10 w-10 items-center justify-center rounded-lg mb-3" style={{ backgroundColor: theme.primary15 }}><Sparkles className="h-5 w-5" style={{ color: theme.primary }} /></div><p className="font-medium text-sm" style={{ color: theme.text }}>{ind.label}</p><p className="text-sm mt-0.5" style={{ color: theme.textMuted }}>{ind.description}</p><span className="text-[11px] font-medium px-2 py-0.5 rounded-full inline-block mt-2" style={{ backgroundColor: theme.primary15, color: theme.primary }}>AI-built</span><p className="text-sm mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: theme.primary }}>Configure package <ArrowUpRight className="h-2.5 w-2.5" /></p></Link>); } return (<Link key={ind.frontendKey} href={`/agency/templates/${ind.frontendKey}`} className="rounded-xl p-4 transition-all group" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }} onMouseEnter={e => (e.currentTarget.style.borderColor = theme.primary + '60')} onMouseLeave={e => (e.currentTarget.style.borderColor = theme.border)}><div className="flex items-start justify-between mb-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ backgroundColor: ind.hasCustomTemplate ? theme.primary15 : theme.hover }}><Ic className="h-5 w-5" style={{ color: ind.hasCustomTemplate ? theme.primary : theme.textMuted }} /></div>{ind.hasCustomTemplate ? (<span className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: theme.primary15, color: theme.primary }}><Check className="h-2.5 w-2.5" /> Custom</span>) : (<span className="text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: theme.hover, color: theme.textMuted }}>Default</span>)}</div><p className="font-medium text-sm" style={{ color: theme.text }}>{ind.label}</p><p className="text-sm mt-0.5" style={{ color: theme.textMuted }}>{ind.description}</p><p className="text-sm mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: theme.primary }}>Configure package <ArrowUpRight className="h-2.5 w-2.5" /></p></Link>); })}<button onClick={() => { setShowAddIndustry(true); setAddIndustryError(''); }} className="rounded-xl p-4 flex flex-col items-center justify-center gap-2 text-center transition-all" style={{ border: `1px dashed ${theme.border}`, minHeight: '132px' }} onMouseEnter={e => (e.currentTarget.style.borderColor = theme.primary + '60')} onMouseLeave={e => (e.currentTarget.style.borderColor = theme.border)}><div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ backgroundColor: theme.hover }}><Plus className="h-5 w-5" style={{ color: theme.primary }} /></div><span className="text-sm font-medium" style={{ color: theme.text }}>Add industry</span><span className="text-sm" style={{ color: theme.textMuted }}>Create a custom vertical. AI builds the knowledge base.</span></button></div></div>
            )}
            {!clientsLoading && clients.length > 0 && (<p className="text-sm text-center" style={{ color: theme.textMuted }}>Select a client above to configure their AI receptionist</p>)}
          </div>
        )}
        ...
        {selectedClient && (
          <div>
              ...
              <div className="rounded-xl p-4 sm:p-5 mb-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea value={editPrompt} onChange={e => setEditPrompt(e.target.value)} rows={promptExpanded ? 30 : 6} className="w-full rounded-lg px-3 py-2.5 text-sm font-mono leading-relaxed transition-all" style={{ ...inputStyle, resize: 'vertical', minHeight: promptExpanded ? '400px' : '120px', maxHeight: promptExpanded ? 'none' : '200px' }} />
                  </div>
            ...
            {selectedClient?.vapi_assistant_id && (
              <div className="rounded-xl p-4 sm:p-5 mb-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
                <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><BookOpen className="h-4 w-4" style={{ color: theme.primary }} /><span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>Knowledge Base</span>{kbLastUpdated && (<span className="text-[11px] px-1.5 py-0.5 rounded" style={{ backgroundColor: theme.hover, color: theme.textMuted }}>Updated {new Date(kbLastUpdated).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>)}</div><button onClick={() => setKbExpanded(!kbExpanded)} className="flex items-center gap-1 text-sm font-medium transition" style={{ color: theme.primary }}>{kbExpanded ? 'Hide' : 'Edit'}<ChevronDown className={`h-3.5 w-3.5 transition-transform ${kbExpanded ? 'rotate-180' : ''}`} /></button></div>
                {kbLoading ? (<div className="flex items-center justify-center py-6"><Loader2 className="h-5 w-5 animate-spin" style={{ color: theme.primary }} /><span className="ml-2 text-sm" style={{ color: theme.textMuted }}>Loading knowledge base...</span></div>
                ) : !kbExpanded ? (<p className="text-sm" style={{ color: theme.textMuted }}>{kbServices.filter(s => s.name.trim()).length} services · {kbFaqs.filter(f => f.question.trim()).length} FAQs · {kbAdditionalInfo ? 'Has additional info' : 'No additional info'}</p>
                ) : (
                  <div className="space-y-5 mt-3">
                    <div><label className="flex items-center gap-1.5 text-sm font-medium mb-1.5" style={{ color: theme.textMuted }}><Globe className="w-3.5 h-3.5" style={{ color: theme.primary }} /> Website</label><input type="url" value={kbWebsite} onChange={e => setKbWebsite(e.target.value)} placeholder="https://yourbusiness.com" className="w-full rounded-lg px-3 py-2 text-sm" style={inputStyle} /></div>
                    <div><label className="flex items-center gap-1.5 text-sm font-medium mb-1.5" style={{ color: theme.textMuted }}><Briefcase className="w-3.5 h-3.5" style={{ color: theme.primary }} /> Services & Pricing</label><div className="space-y-2">{kbServices.map(service => (<div key={service.id} className="p-2.5 rounded-lg space-y-1.5" style={{ backgroundColor: theme.isDark ? 'rgba(255,255,255,0.02)' : '#f9fafb' }}><div className="flex gap-2"><input type="text" value={service.name} onChange={e => updateKbService(service.id, 'name', e.target.value)} placeholder="Service name" className="flex-1 rounded-lg px-2.5 py-1.5 text-sm min-w-0" style={inputStyle} /><input type="text" value={service.price} onChange={e => updateKbService(service.id, 'price', e.target.value)} placeholder="$100" className="w-20 rounded-lg px-2.5 py-1.5 text-sm" style={inputStyle} /><button onClick={() => removeKbService(service.id)} disabled={kbServices.length === 1} className="p-1.5 transition disabled:opacity-20" style={{ color: theme.textMuted }}><Trash2 className="w-3.5 h-3.5" /></button></div><textarea value={service.description} onChange={e => updateKbService(service.id, 'description', e.target.value)} placeholder="Brief description..." rows={1} className="w-full rounded-lg px-2.5 py-1.5 text-sm resize-none" style={inputStyle} /></div>))}</div><button onClick={addKbService} className="mt-2 flex items-center gap-1 px-2.5 py-1.5 text-sm font-medium rounded-lg transition hover:opacity-80" style={{ color: theme.primary, backgroundColor: hexToRgba(theme.primary, 0.08) }}><Plus className="w-3 h-3" /> Add Service</button></div>
                    <div><label className="flex items-center gap-1.5 text-sm font-medium mb-1.5" style={{ color: theme.textMuted }}><HelpCircle className="w-3.5 h-3.5" style={{ color: theme.primary }} /> FAQs</label><div className="space-y-2">{kbFaqs.map(faq => (<div key={faq.id} className="p-2.5 rounded-lg space-y-1.5" style={{ backgroundColor: theme.isDark ? 'rgba(255,255,255,0.02)' : '#f9fafb' }}><div className="flex gap-2"><input type="text" value={faq.question} onChange={e => updateKbFaq(faq.id, 'question', e.target.value)} placeholder="Question..." className="flex-1 rounded-lg px-2.5 py-1.5 text-sm min-w-0" style={inputStyle} /><button onClick={() => removeKbFaq(faq.id)} disabled={kbFaqs.length === 1} className="p-1.5 transition disabled:opacity-20" style={{ color: theme.textMuted }}><Trash2 className="w-3.5 h-3.5" /></button></div><textarea value={faq.answer} onChange={e => updateKbFaq(faq.id, 'answer', e.target.value)} placeholder="Answer..." rows={2} className="w-full rounded-lg px-2.5 py-1.5 text-sm resize-none" style={inputStyle} /></div>))}</div><button onClick={addKbFaq} className="mt-2 flex items-center gap-1 px-2.5 py-1.5 text-sm font-medium rounded-lg transition hover:opacity-80" style={{ color: theme.primary, backgroundColor: hexToRgba(theme.primary, 0.08) }}><Plus className="w-3 h-3" /> Add FAQ</button></div>
                    <div><label className="flex items-center gap-1.5 text-sm font-medium mb-1.5" style={{ color: theme.textMuted }}><FileText className="w-3.5 h-3.5" style={{ color: theme.primary }} /> Additional Info</label><textarea value={kbAdditionalInfo} onChange={e => setKbAdditionalInfo(e.target.value)} placeholder="Policies, service areas, payment methods, etc." rows={3} className="w-full rounded-lg px-3 py-2 text-sm resize-none" style={inputStyle} /></div>
                    <button onClick={saveKB} disabled={kbSaving} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl transition disabled:opacity-50" style={{ backgroundColor: theme.primary, color: theme.primaryText }}>{kbSaving ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Updating...</> : <><Sparkles className="h-3.5 w-3.5" /> Update Knowledge Base</>}</button>
                  </div>
                )}
              </div>
            )}
            ...
            {selectedClient?.vapi_assistant_id && (
              <div className="rounded-xl p-4 sm:p-5 mb-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
                <div className="flex items-center gap-2 mb-4"><Bot className="h-4 w-4" style={{ color: theme.primary }} /><span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>AI Behavior</span></div>
            ...
            <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
              <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><Phone className="h-4 w-4" style={{ color: theme.primary }} /><span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.textMuted }}>SMS Notifications Go To</span></div>{phoneSwapped && <button onClick={revertPhone} disabled={phoneSwapping} className="flex items-center gap-1 text-sm font-medium px-2 py-1 rounded-full disabled:opacity-50" style={{ backgroundColor: theme.isDark ? 'rgba(251,191,36,0.1)' : '#fffbeb', color: '#f59e0b', border: `1px solid ${theme.isDark ? 'rgba(251,191,36,0.2)' : '#fde68a'}` }}><Undo2 className="h-3 w-3" /> Revert</button>}</div>
              <div className="flex gap-2"><input type="tel" value={phoneEditing ? notifPhone : fmtPhone(notifPhone)} onChange={e => setNotifPhone(e.target.value)} onFocus={() => setPhoneEditing(true)} className="flex-1 rounded-lg px-3 py-2 text-sm font-mono" style={inputStyle} /><button onClick={swapPhone} disabled={phoneSwapping || !notifPhone.trim()} className="flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium disabled:opacity-40" style={{ backgroundColor: theme.primary, color: theme.primaryText }}>{phoneSwapping ? <Loader2 className="h-3 w-3 animate-spin" /> : <Check className="h-3 w-3" />} Save</button></div>
              <p className="text-sm mt-2" style={{ color: theme.textMuted }}>{phoneSwapped ? `Swapped from owner (${fmtPhone(origPhone)}). Revert after testing.` : 'Change to your number to receive test call SMS summaries.'}</p>
            </div>
          </div>
        )}
      </div>
      {showAddIndustry && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={() => { if (!addingIndustry) setShowAddIndustry(false); }}>
          <div className="rounded-2xl p-5 sm:p-6 w-full max-w-md" style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}` }} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm mb-4" style={{ color: theme.textMuted }}>Name your vertical and describe it in a sentence. We generate the receptionist knowledge base for it, and you can refine it after.</p>
            <label className="block text-sm font-medium mb-1.5" style={{ color: theme.textMuted }}>Industry name</label>
            ...
            <label className="block text-sm font-medium mb-1.5" style={{ color: theme.textMuted }}>Description (optional)</label>
            ...
            {addIndustryError && (<p className="text-sm mb-3" style={{ color: '#ef4444' }}>{addIndustryError}</p>)}
