...
interface ExpandedData { clients: any[]; billable_client_count: number; sms_history: any[]; checklist: { items: Record<string, { done: boolean; label: string }>; done: number; total: number; complete: boolean }; test_client: { id: string; phone: string; calls_used: number; call_limit: number; status: string } | null; referral_chain: { referred_by: string | null; referred_agencies: any[]; earnings_cents: number }; activation: { step: number; last_sent: string | null; onboarding_completed_at: string | null }; onboarding_email: { step: number; last_sent: string | null } }
                ...
                {expandedRow === agency.id && (<tr key={`${agency.id}-detail`}><td colSpan={10} className="px-6 py-0"><div className="py-5 border-t border-[var(--a-line)]">
                  ...
                  {expandedData[agency.id] && (
                    <div className="mt-5 pt-5 border-t border-[var(--a-line)] space-y-5">
                      {/* Onboarding Emails (sent manually via Gmail as support@) */}
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <Mail className="h-3.5 w-3.5" style={{ color: 'var(--a-em-deep)' }} />
                          <h4 className={label}>Onboarding Emails</h4>
                          <span className="text-[10px] text-[var(--a-dim)]">{Math.min(expandedData[agency.id].onboarding_email?.step || 0, ONBOARDING_EMAILS.length)} of {ONBOARDING_EMAILS.length} sent</span>
                        </div>
                        <div className="space-y-1.5">
                          {ONBOARDING_EMAILS.map((tpl) => {
                            const sent = (expandedData[agency.id].onboarding_email?.step || 0) >= tpl.step;
                            return (
                              <div key={tpl.step} className="flex items-center gap-3 rounded-lg px-3 py-2 bg-white border border-[var(--a-line)]">
                                <span className="h-6 w-6 shrink-0 rounded-md flex items-center justify-center text-[11px] font-semibold" style={sent ? { background: 'var(--a-em-soft)', color: 'var(--a-em-deep)' } : { background: 'var(--a-line)', color: 'var(--a-dim)' }}>{tpl.step}</span>
                                <div className="min-w-0 flex-1">
                                  <p className="text-[12px] font-semibold text-[var(--a-ink)] truncate">{tpl.title}</p>
                                  <p className="text-[11px] text-[var(--a-dim)] truncate">{tpl.subject}</p>
                                </div>
                                {sent && <span className="shrink-0 text-[10px] font-semibold" style={{ color: 'var(--a-em-deep)' }}>Sent</span>}
                                <a href={gmailComposeUrl(agency.email, tpl.subject, tpl.body(agency.name || 'there'))} target="_blank" rel="noopener noreferrer" onClick={() => logOnboardingEmail(agency.id, tpl.step)} className="shrink-0 inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold transition-colors hover:brightness-95" style={{ background: 'var(--a-em)', color: '#04140D' }}>
                                  <Mail className="h-3 w-3" />{sent ? 'Resend' : 'Send'}
                                </a>
                              </div>
                            );
                          })}
                        </div>
                        <p className="text-[10px] text-[var(--a-dim)] mt-1.5">Opens Gmail composing as support@myvoiceaiconnect.com. The welcome email already sends automatically on signup.</p>
                      </div>
