...
export function ClientSettingsContent({ client: initialClient, branding }: Props) {
  ...
  useEffect(() => { const fetchCalendarStatus = async () => { try { const response = await fetch(`${backendUrl}/api/auth/google-calendar/status/${client.id}`); if (response.ok) { const data = await response.json(); setCalendarStatus(data); if (data.connected !== client.google_calendar_connected) { setClient(prev => ({ ...prev, google_calendar_connected: data.connected })); } } } catch (err) { console.error('Failed to fetch calendar status:', err); setCalendarStatus({ connected: client.google_calendar_connected || false, token_valid: false, plan_allowed: true, plan_message: null }); } finally { setCalendarLoading(false); } }; fetchCalendarStatus(); }, [client.id, backendUrl]);

  useEffect(() => { const params = new URLSearchParams(window.location.search); if (params.get('success') === 'calendar_connected') { setMessage('Google Calendar connected successfully!'); setClient(prev => ({ ...prev, google_calendar_connected: true })); setCalendarStatus(prev => prev ? { ...prev, connected: true, token_valid: true } : null); window.history.replaceState({}, '', window.location.pathname); setTimeout(() => setMessage(''), 4000); } const error = params.get('error'); if (error === 'plan_upgrade_required') { setMessage('Calendar integration requires a higher plan. Please contact your provider to upgrade.'); window.history.replaceState({}, '', window.location.pathname); setTimeout(() => setMessage(''), 5000); } else if (error === 'calendar_denied') { setMessage('Google Calendar access was denied. Please try again and grant calendar permissions.'); window.history.replaceState({}, '', window.location.pathname); setTimeout(() => setMessage(''), 5000); } else if (error?.startsWith('calendar_')) { setMessage('Failed to connect Google Calendar. Please try again.'); window.history.replaceState({}, '', window.location.pathname); setTimeout(() => setMessage(''), 5000); } }, []);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 pb-24 min-h-screen" style={{ backgroundColor: theme.bg }}>
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <p className="mt-1 text-sm" style={{ color: theme.textMuted }}>Account, integrations, and billing</p>
      </div>
      ...
      <div className="max-w-3xl">
        ...
        {/* Integrations */}
        <section className="mb-4 sm:mb-6">
          <h2 className="text-sm sm:text-base font-semibold mb-2 sm:mb-3 flex items-center gap-2" style={{ color: theme.text }}><Link2 className="w-4 h-4" style={{ color: theme.primary }} />Integrations</h2>
          <div className="rounded-xl border p-3 sm:p-4 shadow-sm" style={{ borderColor: theme.border, backgroundColor: theme.card }}>
            <div className="flex items-center gap-2 sm:gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2"><h3 className="font-semibold text-xs sm:text-sm" style={{ color: theme.text }}>Google Calendar</h3>{isCalendarConnected && (<span className="px-1.5 py-0.5 rounded-full text-[10px] font-semibold" style={{ backgroundColor: theme.successBg, color: theme.success }}>Connected</span>)}</div>
                <p className="text-[10px] sm:text-xs" style={{ color: theme.textMuted4 }}>{isCalendarConnected ? 'Your AI can book appointments to your calendar' : 'Let your AI book appointments to your calendar'}</p>
              </div>
            </div>
            {calendarLoading ? (<div className="flex items-center justify-center py-3"><Loader2 className="w-4 h-4 animate-spin" style={{ color: theme.textMuted4 }} /></div>
            ) : !isCalendarPlanAllowed ? (
              <div className="space-y-2.5">
                <div className="p-2.5 sm:p-3 rounded-lg flex items-start gap-2" style={{ backgroundColor: theme.warningBg, border: `1px solid ${theme.warningBorder}` }}><AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: theme.warning }} /><div><p className="text-xs sm:text-sm font-medium" style={{ color: theme.warningText }}>Calendar integration is not included in your current plan.</p></div></div>
              ...
              <div className="space-y-2.5">
                {calendarStatus && !calendarStatus.token_valid && (<div className="p-2.5 sm:p-3 rounded-lg flex items-start gap-2" style={{ backgroundColor: theme.warningBg, border: `1px solid ${theme.warningBorder}` }}><AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: theme.warning }} /><div><p className="text-xs sm:text-sm font-medium" style={{ color: theme.warningText }}>Calendar connection may have expired.</p></div></div>)}
                <div className="flex gap-2"><button onClick={handleConnectCalendar} className="flex-1 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-semibold transition hover:opacity-90" style={{ backgroundColor: theme.bg, color: theme.textMuted, border: `1px solid ${theme.border}` }}>Reconnect</button><button onClick={handleDisconnectCalendar} disabled={disconnectingCalendar} className="flex-1 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-semibold transition hover:opacity-90 disabled:opacity-50" style={{ backgroundColor: theme.errorBg, color: theme.error, border: `1px solid ${theme.errorBorder}` }}>{disconnectingCalendar ? (<span className="flex items-center justify-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Disconnecting...</span>) : 'Disconnect'}</button></div>
              </div>
            ) : (
