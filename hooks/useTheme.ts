...
function autoCardHover(cardBg: string, isDark: boolean): string {
  if (isLightColor(cardBg)) return '#f3f4f6';
...
/**
 * Check if primary has enough contrast against a background to be visible.
 * Returns true if the luminance difference is > 0.25 (roughly readable).
 */
function hasSufficientContrast(fg: string, bg: string): boolean {
  return Math.abs(getLuminance(fg) - getLuminance(bg)) > 0.25;
}
...
function buildDarkTheme(primary: string, overrides?: BrandingOverrides): Theme {
  ...
  // Active item contrast check: if primary is too close to sidebar bg, use white
  const primaryVisibleOnNav = hasSufficientContrast(primary, navBg);
  const activeItemColor = primaryVisibleOnNav ? primary : '#ffffff';
  const activeItemBg = primaryVisibleOnNav ? withAlpha(primary, 0.15) : 'rgba(255,255,255,0.1)';
  ...
  return {
    ...
    sidebarActiveItemBg: activeItemBg,
    sidebarActiveItemColor: activeItemColor,
