...
export default function BYOTSettings({ agencyId, planType, subscriptionStatus, theme }: BYOTSettingsProps) {
  ...
  const hasAccess = effectivePlan === 'enterprise' || effectivePlan === 'professional' || effectivePlan === 'pro' || effectivePlan === 'scale';
  ...
  if (status?.byot_enabled && status?.has_credentials) {
    return (
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="rounded-xl p-4 sm:p-5" style={{ backgroundColor: theme.primary + '08', border: `1px solid ${theme.primary}30` }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: theme.primary + '15' }}>
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: theme.textMuted }}>
                  ...
                  {status.verified_at && (<> · Verified {new Date(status.verified_at).toLocaleDateString()}</>)}
                </p>
              </div>
            </div>
            <button onClick={handleRemove} disabled={removing} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-medium transition-colors disabled:opacity-50" style={{ backgroundColor: theme.errorBg, border: `1px solid ${theme.errorBorder}`, color: theme.errorText }}>
        ...
        <div className="rounded-xl p-4 flex items-start gap-3" style={{ backgroundColor: theme.infoBg, border: `1px solid ${theme.infoBorder}` }}>
          <div className="mt-0.5 flex-shrink-0"><AlertCircle className="h-4 w-4" style={{ color: theme.infoText }} /></div>
          <div>
            <p className="text-sm font-medium" style={{ color: theme.infoText }}>Regulatory Bundle Required</p>
            <p className="text-xs mt-1" style={{ color: theme.textMuted }}>
        ...
        <div className="rounded-xl p-4 sm:p-5" style={{ backgroundColor: theme.input, border: `1px solid ${theme.inputBorder}` }}>
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              ...
              <select value={testCountry} onChange={(e) => setTestCountry(e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm transition-colors" style={{ backgroundColor: theme.isDark ? '#050505' : '#f9fafb', border: `1px solid ${theme.inputBorder}`, color: theme.text }}>
            ...
            <button onClick={handleTest} disabled={testing} className="rounded-xl px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50" style={{ backgroundColor: theme.primary, color: theme.primaryText }}>
              {testing ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Search'}
            </button>
          </div>

          {testResult && (
            <div className="mt-4 rounded-xl p-3" style={{ backgroundColor: testResult.success ? theme.primary + '08' : theme.warningBg, border: `1px solid ${testResult.success ? theme.primary + '30' : theme.warningBorder}` }}>
              {testResult.success ? (
                <>
                ...
                <>
                  ...
                  {testResult.error === 'regulatory_bundle_required' && testResult.twilio_url && (
                    <a href={testResult.twilio_url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs underline" style={{ color: theme.warningText }}>
        ...
        {message && (
          <div className="rounded-xl p-3 text-xs" style={{ backgroundColor: message.type === 'success' ? theme.primary + '08' : theme.errorBg, border: `1px solid ${message.type === 'success' ? theme.primary + '30' : theme.errorBorder}`, color: message.type === 'success' ? theme.primary : theme.errorText }}>
            {message.text}
          </div>
        )}
      </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="rounded-xl p-4" style={{ backgroundColor: theme.infoBg, border: `1px solid ${theme.infoBorder}` }}>
        ...
        <ol className="text-xs space-y-1.5 list-decimal list-inside" style={{ color: theme.infoText }}>
          ...
          <li>Go to{' '}<a href="https://console.twilio.com/us1/account/keys-credentials/api-keys" target="_blank" rel="noopener noreferrer" className="underline">API Keys &amp; Tokens</a>{' '}→ Create a Standard API Key</li>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div>
          ...
          <input type="text" value={accountSid} onChange={(e) => setAccountSid(e.target.value.trim())} placeholder="AC..." className="w-full rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-mono transition-colors" style={{ backgroundColor: theme.input, border: `1px solid ${theme.inputBorder}`, color: theme.text }} required />
        </div>
        <div>
          <label className="block text-xs sm:text-sm font-medium mb-1.5">API Key SID</label>
          <input type="text" value={apiKey} onChange={(e) => setApiKey(e.target.value.trim())} placeholder="SK..." className="w-full rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-mono transition-colors" style={{ backgroundColor: theme.input, border: `1px solid ${theme.inputBorder}`, color: theme.text }} required />
        </div>
        <div>
          <label className="block text-xs sm:text-sm font-medium mb-1.5">API Secret</label>
          <input type="password" value={apiSecret} onChange={(e) => setApiSecret(e.target.value.trim())} placeholder="Enter your API Secret" className="w-full rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-sm font-mono transition-colors" style={{ backgroundColor: theme.input, border: `1px solid ${theme.inputBorder}`, color: theme.text }} required />
        ...
        <button type="submit" disabled={saving || !accountSid || !apiKey || !apiSecret} className="w-full rounded-xl px-4 py-2.5 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed" style={{ backgroundColor: theme.primary, color: theme.primaryText }}>
          {saving ? (<span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Verifying & Saving...</span>) : 'Connect Twilio Account'}
        </button>
      </form>

      {message && (
        <div className="rounded-xl p-3 flex items-center gap-2 text-xs" style={{ backgroundColor: message.type === 'success' ? theme.primary + '08' : theme.errorBg, border: `1px solid ${message.type === 'success' ? theme.primary + '30' : theme.errorBorder}`, color: message.type === 'success' ? theme.primary : theme.errorText }}>
