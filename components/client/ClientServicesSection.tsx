...
export default function ClientServicesSection({ clientId, theme, compact }: Props) {
  ...
  const openAdd = () => {
    setForm({ ...EMPTY_FORM, assigned_staff: [] });
    setEditingId(null);
    setError('');
    setShowModal(true);
  };
  ...
  const openEdit = (svc: ServiceItem) => {
    setForm({
      ...
      booking_mode: svc.booking_mode,
      assigned_staff: svc.assigned_staff || [],
    });
    setEditingId(svc.id);
    setError('');
    setShowModal(true);
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const url = editingId
        ? `${backendUrl}/api/client/${clientId}/services/${editingId}`
        : `${backendUrl}/api/client/${clientId}/services`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      await fetch(`${backendUrl}/api/client/${clientId}/services/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchServices();
    ...
    <section className={compact ? '' : 'mb-4 sm:mb-6'}>
      ...
      <div className={compact ? '' : 'rounded-xl border p-3 sm:p-4 shadow-sm'} style={compact ? {} : { borderColor: theme.border, backgroundColor: theme.card }}>
        {loading ? (
          <div className="flex items-center justify-center py-6">
            ...
            <span className="ml-2 text-xs" style={{ color: theme.textMuted }}>Loading services...</span>
          </div>
        ) : (
          <>
            {services.length === 0 ? (
                ...
                {services.map(svc => {
                  ...
                  return (
                    <div
                      ...
                      style={{ ...glass, opacity: svc.is_active ? 1 : 0.5 }}
                    >
                      {/* Icon */}
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-xs sm:text-sm font-medium truncate" style={{ color: theme.text }}>{svc.name}</p>
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ backgroundColor: modeStyle.bg, color: modeStyle.color }}>
                            {modeStyle.label}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                          <span className="text-[10px] flex items-center gap-1" style={{ color: theme.textMuted }}>
                            <Clock className="h-2.5 w-2.5" />{svc.duration_minutes}min
                          </span>
                          {svc.buffer_minutes > 0 && (
                            <span className="text-[10px] flex items-center gap-1" style={{ color: theme.textMuted }}>
                              <Shield className="h-2.5 w-2.5" />{svc.buffer_minutes}min buffer
                            </span>
                          )}
                          {assignedNames.length > 0 && (
                            <span className="text-[10px] flex items-center gap-1" style={{ color: theme.textMuted }}>
                              <Users className="h-2.5 w-2.5" />{assignedNames.join(', ')}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button onClick={() => openEdit(svc)} className="p-1.5 rounded-lg transition hover:opacity-70" style={{ color: theme.textMuted }} title="Edit">
                          <Pencil className="h-3.5 w-3.5" />
                        </button>
                        <button onClick={() => handleDelete(svc.id)} disabled={deletingId === svc.id} className="p-1.5 rounded-lg transition hover:opacity-70" style={{ color: '#ef4444' }} title="Remove">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => setShowModal(false)}>
          ...
          <div
            className="relative w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 max-h-[85vh] overflow-y-auto"
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text" placeholder="e.g. General Checkup, Haircut, Consultation"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setForm({ ...form, buffer_minutes: parseInt(e.target.value) })}
                    className="w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none cursor-pointer"
                    style={inputStyle}
                  >
              ...
              <div>
                ...
                <div className="space-y-2">
                  {BOOKING_MODES.map(mode => (
                    <button
                      key={mode.value}
                      onClick={() => setForm({ ...form, booking_mode: mode.value })}
                      className="w-full text-left p-3 rounded-xl border-2 transition"
                      style={{
                        borderColor: form.booking_mode === mode.value ? theme.primary : (theme.isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'),
                        backgroundColor: form.booking_mode === mode.value ? hexToRgba(theme.primary, theme.isDark ? 0.08 : 0.03) : 'transparent',
                      }}
                    >
                      <div className="flex items-center gap-2">
                        <div
                          className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0"
                          style={{ borderColor: form.booking_mode === mode.value ? theme.primary : (theme.isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)') }}
                        >
                          {form.booking_mode === mode.value && <div className="w-2 h-2 rounded-full" style={{ backgroundColor: theme.primary }} />}
                        </div>
              ...
              {allStaff.length > 0 && (
                <div>
                  <label className="block text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: theme.textMuted }}>
                    Assigned Staff <span className="normal-case font-normal">(who performs this service)</span>
                  </label>
                  <div className="flex flex-wrap gap-2">
                    {allStaff.map(s => {
                      const isAssigned = form.assigned_staff.includes(s.id);
                      return (
                        <button
                          key={s.id}
                          ...
                          style={{
                            backgroundColor: isAssigned ? hexToRgba(theme.primary, theme.isDark ? 0.15 : 0.08) : (theme.isDark ? 'rgba(255,255,255,0.04)' : '#f3f4f6'),
                            color: isAssigned ? theme.primary : theme.textMuted,
                            border: `1px solid ${isAssigned ? hexToRgba(theme.primary, 0.3) : 'transparent'}`,
                          }}
                        >
                  ...
                  {form.assigned_staff.length === 0 && (
                    <p className="text-[10px] mt-1" style={{ color: theme.textMuted }}>No staff assigned — any available provider can handle this service</p>
                  )}
                </div>
              )}
